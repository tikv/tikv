...
test:
	...
	cargo test --no-default-features --features "${ENABLE_FEATURES}" --manifest-path tests/Cargo.toml ${EXTRA_CARGO_ARGS} -- --nocapture $(TEST_THREADS) && \
	cargo test --no-default-features --features "${ENABLE_FEATURES}" --manifest-path tests/Cargo.toml --bench misc ${EXTRA_CARGO_ARGS} -- --nocapture  $(TEST_THREADS) && \
	if [[ "`uname`" == "Linux" ]]; then \
