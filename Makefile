...
test:
	...
	cargo test --features "${ENABLE_FEATURES}" --bench raftstore ${EXTRA_CARGO_ARGS} -- --nocapture && \
	cargo test --features "${ENABLE_FEATURES}" --bench rocksdb_writebatch ${EXTRA_CARGO_ARGS} -- --nocapture && \
	cargo test --features "${ENABLE_FEATURES}" --bench storage_scan ${EXTRA_CARGO_ARGS} -- --nocapture && \
	if [[ "`uname`" == "Linux" ]]; then \
