...
fn test_gc_bypass_raft() {
    let (cluster, leader, ctx) = test_raftstore::must_new_cluster_mul(2);
...
// Raftstore-v2 introduces `Tablet` to manage regions, encapsulating the
// operations on KVDB. This case ensures it's performed correctly.
#[test]
fn test_gc_bypass_raft_v2() {
    let (cluster, leader, ctx) = test_raftstore_v2::must_new_cluster_mul(2);
    cluster.pd_client.disable_default_operator();
    ...
    for &start_ts in &[10, 20, 30, 40] {
        ...
        let key = Key::from_raw(b"k1").append_ts(commit_ts.into());
        let key = data_key(key.as_encoded());
        assert!(kv_engine.get_value_cf(CF_WRITE, &key).unwrap().is_some());
    }
    ...
    for store_id in node_ids {
        ...
        for &start_ts in &[10, 20, 30] {
            ...
            let key = Key::from_raw(b"k1").append_ts(start_ts.into());
            let key = data_key(key.as_encoded());
            assert!(kv_engine.get_value(&key).unwrap().is_none());

            let key = Key::from_raw(b"k1").append_ts(commit_ts.into());
            let key = data_key(key.as_encoded());
            assert!(kv_engine.get_value_cf(CF_WRITE, &key).unwrap().is_none());
        }
    }
}
