...
fn deadlock_detector_leader_must_be(cluster: &mut Cluster<ServerCluster>, store_id: u64) {
    ...
    let leader_peer = find_peer(&leader_region, store_id).unwrap().clone();
    cluster
        .pd_client
...
fn must_transfer_leader(cluster: &mut Cluster<ServerCluster>, region_key: &[u8], store_id: u64) {
    ...
    let target_peer = find_peer(&region, store_id).unwrap().clone();
...
fn must_transfer_region(
    ...
    let source_peer = find_peer(&region, source_store_id).unwrap().clone();
    cluster
        .pd_client
