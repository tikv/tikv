...
use file_system::{set_io_rate_limiter, IOMeasure, IOOp, IORateLimiter, IOType};
use raftstore::store::*;
...
fn test_inspected_snapshot() {
    ...
    assert_eq!(
        stats.fetch(IOType::Replication, IOOp::Read, IOMeasure::Bytes),
        0
    );
    assert_eq!(
        stats.fetch(IOType::Replication, IOOp::Write, IOMeasure::Bytes),
        0
    );
    ...
    assert_eq!(
        stats.fetch(IOType::LoadBalance, IOOp::Read, IOMeasure::Bytes),
        0
    );
    assert_eq!(
        stats.fetch(IOType::LoadBalance, IOOp::Write, IOMeasure::Bytes),
        0
    );
