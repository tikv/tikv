...
fn test_multi_base<T: Simulator<HybridEngineImpl>>(cluster: &mut Cluster<HybridEngineImpl, T>) {
    cluster.run();

    test_multi_base_after_bootstrap(cluster);
}

fn test_multi_base_after_bootstrap<T: Simulator<HybridEngineImpl>>(
...
fn test_multi_leader_crash<T: Simulator<HybridEngineImpl>>(
    cluster: &mut Cluster<HybridEngineImpl, T>,
) {
    cluster.run();
...
fn test_multi_cluster_restart<T: Simulator<HybridEngineImpl>>(
    cluster: &mut Cluster<HybridEngineImpl, T>,
) {
    cluster.run();
...
fn test_multi_lost_majority<T: Simulator<HybridEngineImpl>>(
    cluster: &mut Cluster<HybridEngineImpl, T>,
    count: usize,
) {
    cluster.run();
...
fn test_multi_random_restart<T: Simulator<HybridEngineImpl>>(
    cluster: &mut Cluster<HybridEngineImpl, T>,
    node_count: usize,
    restart_count: u32,
) {
...
fn test_multi_latency<T: Simulator<HybridEngineImpl>>(cluster: &mut Cluster<HybridEngineImpl, T>) {
    cluster.run();
...
fn test_multi_random_latency<T: Simulator<HybridEngineImpl>>(
    cluster: &mut Cluster<HybridEngineImpl, T>,
) {
    cluster.run();
...
fn test_multi_drop_packet<T: Simulator<HybridEngineImpl>>(
    cluster: &mut Cluster<HybridEngineImpl, T>,
) {
    cluster.run();
...
fn test_leader_change_with_uncommitted_log<T: Simulator<HybridEngineImpl>>(
    cluster: &mut Cluster<HybridEngineImpl, T>,
) {
...
fn test_read_leader_with_unapplied_log<T: Simulator<HybridEngineImpl>>(
    cluster: &mut Cluster<HybridEngineImpl, T>,
) {
    cluster.cfg.raft_store.raft_election_timeout_ticks = 50;
    // disable compact log to make test more stable.
...
fn get_with_timeout<T: Simulator<HybridEngineImpl>>(
    cluster: &mut Cluster<HybridEngineImpl, T>,
    key: &[u8],
    read_quorum: bool,
    timeout: Duration,
...
fn test_remove_leader_with_uncommitted_log<T: Simulator<HybridEngineImpl>>(
    cluster: &mut Cluster<HybridEngineImpl, T>,
) {
    cluster.cfg.raft_store.raft_election_timeout_ticks = 50;
    // disable compact log to make test more stable.
...
fn test_consistency_check<T: Simulator<HybridEngineImpl>>(
    cluster: &mut Cluster<HybridEngineImpl, T>,
) {
    cluster.cfg.raft_store.raft_election_timeout_ticks = 50;
    // disable compact log to make test more stable.
...
fn test_batch_write<T: Simulator<HybridEngineImpl>>(cluster: &mut Cluster<HybridEngineImpl, T>) {
    cluster.run();
