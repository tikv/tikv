...
fn test_dr_auto_sync() {
    ...
    let rx = cluster
        .async_request(put_request(&region, 1, b"k2", b"v2"))
        .unwrap();
    assert_eq!(
...
fn test_switching_replication_mode() {
    ...
    cluster
        .pd_client
        .switch_replication_mode(DrAutoSyncState::Async, None);
    rx.recv_timeout(Duration::from_millis(100)).unwrap();
    ...
    cluster
        .pd_client
        .switch_replication_mode(DrAutoSyncState::SyncRecover, None);
    thread::sleep(Duration::from_millis(100));
    let rx = cluster
...
// Delay replication mode switch if groups consistent can't reach immediately,
// until groups consistent reached or timeout reached.
#[test]
fn test_delaying_switch_replication_mode() {
    let mut cluster = prepare_cluster();
    let region = cluster.get_region(b"k1");
    cluster.add_send_filter(IsolationFilterFactory::new(3));
    cluster
    ...
    // Replication mode not switch yet, so log entry still can be committed
    cluster
        .pd_client
        .switch_replication_mode(DrAutoSyncState::SyncRecover, Some(1)); // Delay for 1s
    thread::sleep(Duration::from_millis(100));
    cluster.must_put(b"k3", b"v3");
    thread::sleep(Duration::from_millis(100));
    let state = cluster.pd_client.region_replication_status(region.get_id());
    ...
    // Replication mode switch because timeout reached
    ...
    // Replication mode switch because groups consistent reached
    cluster.clear_send_filters();
    cluster
        .pd_client
        .switch_replication_mode(DrAutoSyncState::SyncRecover, Some(1)); // Delay for 1s
