...
fn test_stale_peer_out_of_region<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    let state: RegionLocalState = engine_2.c().get_msg_cf(CF_RAFT, &state_key).unwrap().unwrap();
...
fn test_stale_peer_without_data<T: Simulator>(cluster: &mut Cluster<T>, right_derive: bool) {
    ...
    let state: RegionLocalState = engine3.c().get_msg_cf(CF_RAFT, &state_key).unwrap().unwrap();
    assert_eq!(state.get_state(), PeerState::Tombstone);
