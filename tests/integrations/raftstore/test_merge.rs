...
fn test_merge_pessimistic_locks_propose_fail() {
    ...
    // Filter MsgTimeoutNow, so transfer leader will not actually succeed.
    cluster.add_send_filter(CloneFilterFactory(
    ...
    let from_peer = left.peers.iter().find(|p| p.store_id == 2).unwrap().clone();
    let to_peer = left.peers.iter().find(|p| p.store_id == 1).unwrap().clone();
    let mut msg = eraftpb::Message::default();
    msg.set_msg_type(eraftpb::MessageType::MsgTransferLeader);
    msg.set_log_term(snapshot.term.unwrap().into());
    msg.set_from(from_peer.get_id());
    msg.set_to(to_peer.get_id());
    msg.set_context(vec![1].into());
    let mut message = RaftMessage::default();
    message.set_region_id(left.id);
    message.set_from_peer(from_peer);
    message.set_to_peer(to_peer);
    message.set_region_epoch(left.get_region_epoch().clone());
    message.set_message(msg);
    router.send_raft_message(message).unwrap();

    // Now, the region is in transfer leader state, it will fail to propose the locks
    // or the PrepareMerge request.
    cluster.merge_region(left.id, right.id, Callback::None);
    thread::sleep(Duration::from_millis(150));
