...
fn assert_same_files(mut files1: Vec<kvproto::brpb::File>, mut files2: Vec<kvproto::brpb::File>) {
    ...
    for i in 0..files1.len() {
        ...
        // generated file. The SHA will not never be the same. Detail: https://github.com/facebook/rocksdb/pull/6983
        f1.sha256.clear();
        f2.sha256.clear();
        assert_eq!(f1, f2);
    }
...
fn test_backup_raw_meta_impl(cur_api_version: ApiVersion, dst_api_version: ApiVersion) {
    ...
    // assert_eq!(total_size, 1619); // the number changed when kv size change,
    // should not be an test points. please update this number (must be > 0)
