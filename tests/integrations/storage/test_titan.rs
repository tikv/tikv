...
use raftstore::store::{apply_sst_cf_files_by_ingest, CfFile, RegionSnapshot};
...
// engine_rocks::util::new_engine_opt(path.path().to_str().unwrap(), kv_db_opts,
// kv_cfs_opts) .unwrap(),
// engine_rocks::util::new_engine(raft_path.to_str().unwrap(),
// &[CF_DEFAULT]).unwrap(), );
//
...
// db.ingest_external_file_cf(cf_default, &opts,
// &[sst_file_path.to_str().unwrap()]) .unwrap();
//
...
// .map(|s| s.as_str())
// .collect::<Vec<&str>>();
// apply_sst_cf_files_by_ingest(&tmp_file_paths, &engines1.kv,
// CF_DEFAULT).unwrap(); let tmp_file_paths = cf_file_write.tmp_file_paths();
// let tmp_file_paths = tmp_file_paths
// .iter()
// .map(|s| s.as_str())
// .collect::<Vec<&str>>();
