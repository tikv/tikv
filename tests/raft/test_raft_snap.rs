...
fn test_sending_snapshot_set_pending_snapshot() {
    ...
    sm.prs.voters.get_mut(&2).unwrap().next_idx = sm.raft_log.first_index();
...
fn test_pending_snapshot_pause_replication() {
    ...
    sm.prs.voters.get_mut(&2).unwrap().become_snapshot(11);
...
fn test_snapshot_failure() {
    ...
    sm.prs.voters.get_mut(&2).unwrap().next_idx = 1;
    sm.prs.voters.get_mut(&2).unwrap().become_snapshot(11);
