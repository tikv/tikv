...
use rocksdb::DB;
use protobuf;
use kvproto::raft_serverpb::{RaftApplyState, RaftTruncatedState, RaftLocalState};
...
fn test_compact_size_limit<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    for (&id, engine) in &cluster.engines {
        ...
        let before_state = &before_states[&id];
        let idx = after_state.get_index();
        if id == (&cluster.leaders[&1]).get_id() {
            assert_eq!(idx, before_state.get_index());
        } else {
...
fn test_local_compact_log<T: Simulator>(cluster: &mut Cluster<T>) {
    cluster.cfg.raft_store.raft_log_gc_count_limit = 100000;
    cluster.cfg.raft_store.raft_log_gc_size_limit = 2 * 1024 * 1024;
    cluster.cfg.raft_store.raft_log_gc_threshold = 500;
    cluster.run();
    ...
    // leader limit has not reached, should not gc. but follower can local compact
    for (&id, engine) in &cluster.engines {
        if id == 1 {
            continue;
        }
        ...
        let key = keys::raft_log_key(1, idx + 1);
        assert!(engine.get_cf(handle, &key).unwrap().is_some());

        let raft_state: RaftLocalState =
            get_msg_cf_or_default(engine, CF_RAFT, &keys::raft_state_key(1));
        assert_eq!(idx + 1, raft_state.get_last_index());
    }
}

#[test]
fn test_node_local_compact_log() {
    let count = 5;
    let mut cluster = new_node_cluster(0, count);
    test_local_compact_log(&mut cluster);
}
