...
fn test_tombstone<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    let region_status = new_status_request(1, new_region_leader_cmd());
    let resp = cluster.call_command(2, region_status, Duration::from_secs(3)).unwrap();
    ...
    // Send a stale raft message to peer 2
    let mut raft_msg = raft_serverpb::RaftMessage::new();

    raft_msg.set_region_id(1);
    raft_msg.mut_message().set_from(1);
    raft_msg.mut_message().set_to(2);
