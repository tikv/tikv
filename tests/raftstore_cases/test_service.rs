...
use grpc::{CallOption, ChannelBuilder, Environment, Error, RpcStatusCode};
...
fn test_rawkv() {
    ...
    put_req.key = k.clone();
    put_req.value = v.clone();
    let put_resp = client.raw_put_opt(&put_req, cluster.call_option()).unwrap();
    ...
    get_req.key = k.clone();
    let get_resp = client.raw_get_opt(&get_req, cluster.call_option()).unwrap();
    ...
    let scan_resp = client
        .raw_scan_opt(&scan_req, cluster.call_option())
        .unwrap();
    ...
    let delete_resp = client
        .raw_delete_opt(&delete_req, cluster.call_option())
        .unwrap();
...
fn must_kv_prewrite(
    ...
    let prewrite_resp = client
        .kv_prewrite_opt(&prewrite_req, cluster.call_option())
        .unwrap();
    assert!(
...
fn must_kv_commit(
    ...
    let commit_resp = client
        .kv_commit_opt(&commit_req, cluster.call_option())
        .unwrap();
    assert!(
...
fn test_mvcc_basic() {
    ...
    let get_resp = client.kv_get_opt(&get_req, cluster.call_option()).unwrap();
    assert!(!get_resp.has_region_error());
    ...
    let scan_resp = client
        .kv_scan_opt(&scan_req, cluster.call_option())
        .unwrap();
    assert!(!scan_resp.has_region_error());
    ...
    let batch_get_resp = client
        .kv_batch_get_opt(&batch_get_req, cluster.call_option())
        .unwrap();
...
fn test_mvcc_rollback_and_cleanup() {
    ...
    let scan_lock_resp = client
        .kv_scan_lock_opt(&scan_lock_req, cluster.call_option())
        .unwrap();
    ...
    let cleanup_resp = client
        .kv_cleanup_opt(&cleanup_req, cluster.call_option())
        .unwrap();
...
fn test_mvcc_resolve_lock_gc_and_delete() {
    ...
    let resolve_lock_resp = client
        .kv_resolve_lock_opt(&resolve_lock_req, cluster.call_option())
        .unwrap();
    ...
    let get_resp1 = client.kv_get_opt(&get_req1, cluster.call_option()).unwrap();
    ...
    let gc_resp = client.kv_gc_opt(&gc_req, cluster.call_option()).unwrap();
    ...
    let get_resp2 = client.kv_get_opt(&get_req2, cluster.call_option()).unwrap();
    ...
    let mvcc_get_by_key_resp = client
        .mvcc_get_by_key_opt(&mvcc_get_by_key_req, cluster.call_option())
        .unwrap();
    ...
    let mvcc_get_by_start_ts_resp = client
        .mvcc_get_by_start_ts_opt(&mvcc_get_by_start_ts_req, cluster.call_option())
        .unwrap();
    ...
    let del_resp = client
        .kv_delete_range_opt(&del_req, cluster.call_option())
        .unwrap();
...
#[test]
fn test_raft() {
    let (cluster, client, _) = must_new_cluster_and_kv_client();

    // Raft commands
    let (sink, _) = client.raft_opt(cluster.call_option()).unwrap();
    sink.send((RaftMessage::new(), Default::default()))
        .wait()
        .unwrap();

    let (sink, _) = client.snapshot_opt(cluster.call_option()).unwrap();
...
fn test_coprocessor() {
    ...
    client
        .coprocessor_opt(&Request::new(), cluster.call_option())
        .unwrap();
}
...
fn test_split_region() {
    ...
    let resp = client
        .split_region_opt(&req, cluster.call_option())
        .unwrap();
    assert_eq!(
...
fn test_debug_get() {
    ...
    req.set_key(key);
    let mut resp = debug_client
        .get_opt(&req, cluster.call_option().clone())
        .unwrap();
    assert_eq!(resp.take_value(), v);

    req.set_key(b"foo".to_vec());
    match debug_client
        .get_opt(&req, cluster.call_option())
        .unwrap_err()
    {
...
fn test_debug_raft_log() {
    ...
    let resp = debug_client
        .raft_log_opt(&req, cluster.call_option())
        .unwrap();
...
fn test_debug_region_info() {
    ...
    req.set_region_id(region_id);
    let mut resp = debug_client
        .region_info_opt(&req, cluster.call_option().clone())
        .unwrap();
    ...
    req.set_region_id(region_id + 1);
    match debug_client
        .region_info_opt(&req, cluster.call_option())
        .unwrap_err()
    {
        Error::RpcFailure(status) => {
            assert_eq!(status.status, RpcStatusCode::NotFound);
...
fn test_debug_region_size() {
    ...
    let entries = debug_client
        .region_size_opt(&req, cluster.call_option())
        .unwrap()
        .take_entries();
...
fn test_debug_fail_point() {
    let (cluster, debug_client, _) = must_new_cluster_and_debug_client();
    ...
    debug_client
        .inject_fail_point_opt(&inject_req, cluster.call_option())
        .unwrap();

    let resp = debug_client
        .list_fail_points_opt(
            &debugpb::ListFailPointsRequest::new(),
            cluster.call_option(),
        )
        .unwrap();
    ...
    debug_client
        .recover_fail_point_opt(&recover_req, cluster.call_option())
        .unwrap();
...
fn test_debug_scan_mvcc() {
    ...
    let receiver = debug_client
        .scan_mvcc_opt(&req, cluster.call_option())
        .unwrap();
