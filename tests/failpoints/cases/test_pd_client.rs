...
fn test_pd_client_deadlock() {
    ...
    let test_funcs: Vec<(_, Box<dyn FnOnce() + Send>)> = vec![
        ...
        request!(client => block_on(ask_batch_split(Region::default(), 1, pdpb::AutoSplitReason::Admin))),
...
fn test_retry() {
    ...
    test_retry_success(&mut client, |c| {
        block_on(c.ask_batch_split(Region::default(), 1, pdpb::AutoSplitReason::Admin))
    });
    test_retry_success(&mut client, |c| {
