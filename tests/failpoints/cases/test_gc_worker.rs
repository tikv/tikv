...
// observer before writing data to the rocksdb:   1. Store-1 transfers a region
// to store-2 and store-2 is applying logs.   2. GC worker registers lock
// observer on store-2 after calling lock observer's callback and before
// finishing applying which means the lock won't be observed.   3. GC worker
// scans locks on each store independently. It's possible GC worker has scanned
// all locks on store-2 and hasn't scanned locks on store-1.   4. Store-2
// applies all logs and removes the peer on store-1.   5. GC worker can't scan
// the lock on store-1 because the peer has been destroyed.   6. GC worker can't
// get the lock from store-2 because it can't observe the lock and has scanned
// it.
#[test]
