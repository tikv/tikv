...
fn test_merge_source_destroy_before_catch_up_logs_leaks_cm_lock() {
    ...
    fail::cfg_callback("on_wait_merge_source_entered", move || {
    ...
    entered_rx
        ...
        .expect("timed out waiting for WaitMergeSource to be entered");
    ...
    // → CM lock cleared.
    //
    // Poll with a generous deadline for robustness on slow machines.
    let cleanup_start = Instant::now();
    loop {
        if cm.global_min_lock_ts().is_none() {
            break;
        }
        if cleanup_start.saturating_elapsed() > Duration::from_secs(30) {
            panic!(
