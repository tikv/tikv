...
macro_rules! readpool_config {
    ($struct_name:ident, $test_mod_name:ident, $display_name:expr) => {
        ...
        pub struct $struct_name {
            pub spark_concurrency: usize,
            pub high_concurrency: usize,
            pub normal_concurrency: usize,
            pub low_concurrency: usize,
            pub max_tasks_per_worker_spark: usize,
        ...
        impl $struct_name {
            ...
            pub fn to_future_pool_configs(&self) -> Vec<future_pool::Config> {
                vec![
                    ...
                    future_pool::Config {
                        workers: self.spark_concurrency,
                        max_tasks_per_worker: self.max_tasks_per_worker_spark,
                        stack_size: self.stack_size.0 as usize,
                    },
                ]
            }
            ...
            pub fn validate(&self) -> Result<(), Box<dyn Error>> {
                if self.spark_concurrency == 0 {
                    return Err(format!(
                        "readpool.{}.spark-concurrency should be > 0",
                        $display_name
                    )
                    .into());
                }
                ...
                if self.max_tasks_per_worker_spark <= 1 {
                    return Err(format!(
                        "readpool.{}.max-tasks-per-worker-spark should be > 1",
                        $display_name
                    )
                    .into());
                }
...
impl Default for StorageReadPoolConfig {
    fn default() -> Self {
        ...
        Self {
            spark_concurrency: concurrency,
            high_concurrency: concurrency,
            normal_concurrency: concurrency,
            low_concurrency: concurrency,
            max_tasks_per_worker_spark: DEFAULT_READPOOL_MAX_TASKS_PER_WORKER,
