...
impl CfConfig {
    fn default_config(is_raft_db: bool) -> CfConfig {
        let mut cfg = CfConfig::default();
        if is_raft_db {
            cfg.use_bloom_filter = false;
            cfg.block_cache_size = ReadableSize::mb(memory_mb_for_cf(true, CF_DEFAULT) as u64);
    ...
    fn write_config() -> CfConfig {
        ...
        cfg.block_cache_size = ReadableSize::mb(memory_mb_for_cf(false, CF_WRITE) as u64);
    ...
    fn lock_config() -> CfConfig {
        ...
        cfg.block_cache_size = ReadableSize::mb(memory_mb_for_cf(false, CF_LOCK) as u64);
    ...
    pub fn build_opt(&self, is_raft_db: bool) -> ColumnFamilyOptions {
        ...
        match (is_raft_db, self.cf.as_str()) {
            (true, CF_DEFAULT) => {
...
impl DbConfig {
    fn new(is_raft_db: bool) -> DbConfig {
        DbConfig {
            ...
            backup_dir: "".to_owned(),
            defaultcf: CfConfig::default_config(is_raft_db),
...
impl DbConfig {
    ...
    pub fn build_cf_opts(&self, is_raft_db: bool) -> Vec<CFOptions> {
        if is_raft_db {
            vec![
                CFOptions::new(CF_DEFAULT, self.defaultcf.build_opt(is_raft_db)),
            ]
        } else {
            vec![
                CFOptions::new(CF_DEFAULT, self.defaultcf.build_opt(is_raft_db)),
                CFOptions::new(CF_LOCK, self.lockcf.build_opt(is_raft_db)),
                CFOptions::new(CF_WRITE, self.writecf.build_opt(is_raft_db)),
            ]
        }
    }
...
impl Default for TiKvConfig {
    fn default() -> TiKvConfig {
        TiKvConfig {
            ...
            pd: PdConfig::default(),
            rocksdb: DbConfig::new(false),
            raftdb: DbConfig::new(true),
