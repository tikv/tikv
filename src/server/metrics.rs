...
make_static_metric! {
    ...
    pub struct SnapTaskCounterVec: IntCounter {
        "type" => {
            recv,
            send,
        },
    ...
    pub struct ResolveStoreCounterVec: IntCounter {
        "type" => {
            resolve,
            failed,
            success,
...
lazy_static! {
    ...
    static ref _SNAP_TASK_COUNTER: IntCounterVec = register_int_counter_vec!(
        "tikv_server_snapshot_task_total",
        "Total number of snapshot task",
        &["type"]
    ).unwrap();
    pub static ref SNAP_TASK_COUNTER: SnapTaskCounterVec =
        SnapTaskCounterVec::from(&_SNAP_TASK_COUNTER);
    static ref _GRPC_MSG_HISTOGRAM_VEC: HistogramVec = register_histogram_vec!(
        "tikv_grpc_msg_duration_seconds",
        "Bucketed histogram of grpc server messages",
        &["type"],
        exponential_buckets(0.0005, 2.0, 20).unwrap()
    ).unwrap();
    pub static ref GRPC_MSG_HISTOGRAM_VEC: GrpcMsgHistogramVec =
        GrpcMsgHistogramVec::from(&_GRPC_MSG_HISTOGRAM_VEC);
    static ref _GRPC_MSG_FAIL_COUNTER: IntCounterVec = register_int_counter_vec!(
    ...
    pub static ref GRPC_MSG_FAIL_COUNTER: GrpcMsgFailCounterVec =
        GrpcMsgFailCounterVec::from(&_GRPC_MSG_FAIL_COUNTER);
    ...
    static ref _RESOLVE_STORE_COUNTER: IntCounterVec = register_int_counter_vec!(
        "tikv_server_resolve_store_total",
        "Total number of resolving store",
        &["type"]
    ).unwrap();
    pub static ref RESOLVE_STORE_COUNTER: ResolveStoreCounterVec =
        ResolveStoreCounterVec::from(&_RESOLVE_STORE_COUNTER);
