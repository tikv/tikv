...
pub struct ServerTransport<T, S> {
    raft_stream_pool: RaftStreamPool<S, T>,
}
    ...
    S: StoreAddrResolver + 'static,
{
    fn clone(&self) -> Self {
        ServerTransport {
            raft_stream_pool: self.raft_stream_pool.clone(),
        }
    }
}

impl<T: RaftStoreRouter + 'static, S: StoreAddrResolver + 'static> ServerTransport<T, S> {
    pub fn new(raft_stream_pool: RaftStreamPool<S, T>) -> ServerTransport<T, S> {
        ServerTransport { raft_stream_pool }
    }
}
    ...
    S: StoreAddrResolver + 'static,
{
    fn send(&mut self, msg: RaftMessage) -> RaftStoreResult<()> {
        self.raft_stream_pool.send(msg)
    }

    fn flush(&mut self) {
        self.raft_stream_pool.flush();
    }
}
