...
impl RequestTask {
    ...
    fn stop_record_waiting(&mut self) {
        ...
        COPR_REQ_WAIT_TIME
            .with_label_values(&["select", get_req_type_str(self.req.get_tp())])
            .observe(wait_time);
    ...
    fn stop_record_handling(&mut self) {
        ...
        COPR_REQ_HISTOGRAM_VEC
            .with_label_values(&["select", type_str])
            .observe(handle_time);
    ...
    fn run_batch(&mut self, tasks: &mut Vec<Task>) {
        ...
        for (_, reqs) in grouped_reqs {
            ...
            if let Err(e) = self.engine
                   .async_snapshot(reqs[0].req.get_context(),
                                   box move |(_, res)| {
    ...
    fn get_rows_from_range(&mut self, range: KeyRange) -> Result<usize> {
        ...
        if is_point(&range) {
            let value = match try!(self.snap
                                       .get(&Key::from_raw(range.get_start()),
                                            &mut self.statistics)) {
                None => return Ok(0),
                Some(v) => v,
            };
            ...
            let mut scanner = try!(self.snap
                                       .scanner(if self.core.desc_scan {
                                                    ScanMode::Backward
                                                } else {
                                                    ScanMode::Forward
                                                },
...
mod tests {
    ...
    fn test_topn_heap_with_cmp_error() {
        ...
        topn_heap
            .try_add_row(0 as i64, order_cols.clone(), ctx.clone(), row_data, std_key)
            .unwrap();
