...
use engine::rocks::{
    CompactOptions, DBBottommostLevelCompaction, DBIterator as RocksIterator, Kv,
    RawReadOptions as ReadOptions, RawSeekKey as SeekKey, RawWriteBatch as WriteBatch,
...
impl Debugger {
    ...
    fn get_db_from_type(&self, db: DBType) -> Result<&DB> {
        match db {
            DBType::Kv => Ok(self.engines.kv.as_ref()),
            DBType::Raft => Ok(self.engines.raft.as_ref()),
    ...
    pub fn scan_mvcc(&self, start: &[u8], end: &[u8], limit: u64) -> Result<MvccInfoIterator> {
        ...
        MvccInfoIterator::new(&self.engines.kv.get_sync_db(), start, end, limit)
    }
    ...
    pub fn raw_scan(
        ...
        let mut iter = RocksIterator::new_cf(db, &cf_handle, read_opt);
    ...
    pub fn set_region_tombstone(&self, regions: Vec<Region>) -> Result<Vec<(u64, Error)>> {
        ...
        if errors.is_empty() {
            let mut write_opts = EngineWriteOptions::new();
            write_opts.set_sync(true);
            box_try!(db.write_opt(&write_opts, &wb));
        }
        Ok(errors)
    }
    ...
    pub fn set_region_tombstone_by_id(&self, regions: Vec<u64>) -> Result<Vec<(u64, Error)>> {
        ...
        let mut write_opts = EngineWriteOptions::new();
        write_opts.set_sync(true);
        db.write_opt(&write_opts, &wb).unwrap();
        Ok(errors)
    }
    ...
    pub fn bad_regions(&self) -> Result<Vec<(u64, Error)>> {
        ...
        let mut iter = DBIterator::new_cf(self.engines.kv.get_sync_db(), handle, readopts.into());
    ...
    pub fn remove_failed_stores(
        ...
        let wb = self.engines.kv.write_batch(0);
...

        {
            ...
            if let Some(region_ids) = region_ids {
                ...
                for region_id in region_ids {
                    let key = keys::region_state_key(region_id);
                    if let Some(value) = box_try!(kv.get_cf(CF_RAFT, &key)) {
        ...
        let mut write_opts = EngineWriteOptions::new();
        write_opts.set_sync(true);
        box_try!(self.engines.kv.write_opt(&write_opts, &wb));
        Ok(())
    }
    ...
    pub fn recreate_region(&self, region: Region) -> Result<()> {
        ...
        let kv_wb = kv.write_batch(0);
        let raft_wb = raft.write_batch(0);
        ...
        let mut write_opts = EngineWriteOptions::new();
        write_opts.set_sync(true);
        box_try!(kv.write_opt(&write_opts, &kv_wb));
        box_try!(raft.write_opt(&write_opts, &raft_wb));
        Ok(())
    }
...
impl MvccChecker {
    fn new(db: Arc<DB>, start_key: &[u8], end_key: &[u8]) -> Result<Self> {
        ...
        let gen_iter = |cf: &str| -> Result<_> {
            ...
            let mut iter = DBIterator::new_cf(Arc::clone(&db), handle, readopts.into());
...
mod tests {
    ...
    use engine::rocks::{DBOptions, RawCFOptions as ColumnFamilyOptions, Rocks, Writable};
    ...
    fn init_region_state(engine: &Rocks, region_id: u64, stores: &[u64]) -> Region {
        let mut region = Region::default();
    ...
    fn get_region_state(engine: &Rocks, region_id: u64) -> RegionLocalState {
        let key = keys::region_state_key(region_id);
        engine
            .get_msg_cf::<RegionLocalState>(CF_RAFT, &key)
            .unwrap()
    ...
    fn test_remove_failed_stores() {
        ...
        let get_region_stores = |engine: &Rocks, region_id: u64| {
    ...
    fn test_bad_regions() {
        ...
        let wb1 = debugger.engines.raft.write_batch(0);
        let wb2 = debugger.engines.kv.write_batch(0);

        {
            let mock_region_state = |region_id: u64, peers: &[u64]| {
                ...
                wb2.put_msg_cf(CF_RAFT, &region_state_key, &region_state)
                    .unwrap();
            };
            let mock_raft_state = |region_id: u64, last_index: u64, commit_index: u64| {
                ...
                wb1.put_msg_cf(CF_DEFAULT, &raft_state_key, &raft_state)
                    .unwrap();
            };
            let mock_apply_state = |region_id: u64, apply_index: u64| {
                ...
                wb2.put_msg_cf(CF_RAFT, &raft_apply_key, &apply_state)
                    .unwrap();
            };
        ...
        raft_engine
            .write_opt(&WriteOptions::default(), &wb1)
            .unwrap();
        kv_engine.write_opt(&WriteOptions::default(), &wb2).unwrap();
    ...
    fn test_modify_tikv_config() {
        ...
        let cf = get_cf_handle(&engine, CF_DEFAULT).unwrap();
    ...
    fn test_debug_raw_scan() {
        ...
        let wb = debugger.engines.kv.write_batch(0);
        for key in keys {
