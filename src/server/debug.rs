...
impl<ER: RaftEngine> Debugger<ER> {
    ...
    pub fn set_region_tombstone(&self, regions: Vec<Region>) -> Result<Vec<(u64, Error)>> {
        ...
        if errors.is_empty() {
            ...
            box_try!(wb.write_opt(&db, &write_opts));
        }
        Ok(errors)
    }
    ...
    pub fn set_region_tombstone_by_id(&self, regions: Vec<u64>) -> Result<Vec<(u64, Error)>> {
        ...
        let mut write_opts = WriteOptions::new();
        write_opts.set_sync(true);
        wb.write_opt(&db, &write_opts).unwrap();
        Ok(errors)
    }
    ...
    pub fn remove_failed_stores(
        ...
        let mut write_opts = WriteOptions::new();
        write_opts.set_sync(true);
        box_try!(wb.write_opt(&self.engines.kv, &write_opts));
        Ok(())
    }
    ...
    pub fn recreate_region(&self, region: Region) -> Result<()> {
        ...
        let mut write_opts = WriteOptions::new();
        write_opts.set_sync(true);
        box_try!(kv_wb.write_opt(&self.engines.kv, &write_opts));
...
fn recover_mvcc_for_range(
    ...
    loop {
        ...
        if !read_only {
            let mut write_opts = WriteOptions::new();
            write_opts.set_sync(true);
            box_try!(wb.write_opt(db.c(), &write_opts));
        } else {
...
mod tests {
    ...
    fn test_bad_regions() {
        ...
        wb1.write_opt(&raft_engine, &WriteOptions::new()).unwrap();
        wb2.write_opt(&kv_engine, &WriteOptions::new()).unwrap();
    ...
    fn test_debug_raw_scan() {
        ...
        wb.write(&debugger.engines.kv).unwrap();
