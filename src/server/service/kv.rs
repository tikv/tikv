...
use crate::{redirect_duplex, redirect_unary};
...
macro_rules! handle_request {
    ($fn_name: ident, $future_name: ident, $req_ty: ident, $resp_ty: ident) => {
        fn $fn_name(&mut self, ctx: RpcContext<'_>, req: $req_ty, sink: UnarySink<$resp_ty>) {
            redirect_unary!(self.proxy, $fn_name, ctx, req, sink);
    ...
    for Service<T, E, L>
{
    ...
    fn coprocessor(&mut self, ctx: RpcContext<'_>, req: Request, sink: UnarySink<Response>) {
        redirect_unary!(self.proxy, coprocessor, ctx, req, sink);
        let begin_instant = Instant::now_coarse();
    ...
    fn split_region(
        ...
        redirect_unary!(self.proxy, split_region, ctx, req, sink);
        let begin_instant = Instant::now_coarse();
    ...
    fn read_index(
        ...
        redirect_unary!(self.proxy, read_index, ctx, req, sink);
        let begin_instant = Instant::now_coarse();
    ...
    fn batch_commands(
        ...
        redirect_duplex!(self.proxy, batch_commands, ctx, stream, sink);
    ...
    fn raw_get_key_ttl(
        ...
        redirect_unary!(self.proxy, raw_get_key_ttl, ctx, req, sink);
        ctx.spawn(
