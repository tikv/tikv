...
use tikv_util::metrics::{ObserveElapsed, TLSExt};
...
impl<T: RaftStoreRouter + 'static, E: Engine, L: LockManager> Tikv for Service<T, E, L> {
        ...
        let future = future_get(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_get.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_scan(&mut self, ctx: RpcContext<'_>, req: ScanRequest, sink: UnarySink<ScanResponse>) {
        ...
        let future = future_scan(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_scan.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_prewrite(
        ...
        let future = future_prewrite(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_prewrite.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_pessimistic_lock(
        ...
        let future = future_acquire_pessimistic_lock(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_pessimistic_lock.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_pessimistic_rollback(
        ...
        let future = future_pessimistic_rollback(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_pessimistic_rollback.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_commit(
        ...
        let future = future_commit(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_commit.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_cleanup(
        ...
        let future = future_cleanup(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_cleanup.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_batch_get(
        ...
        let future = future_batch_get(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_batch_get.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_batch_rollback(
        ...
        let future = future_batch_rollback(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_batch_rollback.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_txn_heart_beat(
        ...
        let future = future_txn_heart_beat(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_txn_heart_beat.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_check_txn_status(
        ...
        let future = future_check_txn_status(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_check_txn_status.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_scan_lock(
        ...
        let future = future_scan_lock(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_scan_lock.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_resolve_lock(
        ...
        let future = future_resolve_lock(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_resolve_lock.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_gc(&mut self, ctx: RpcContext<'_>, req: GcRequest, sink: UnarySink<GcResponse>) {
        ...
        let future = future_gc(&self.gc_worker, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_gc.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn kv_delete_range(
        ...
        let future = future_delete_range(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.kv_delete_range.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn raw_get(
        ...
        let future = future_raw_get(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.raw_get.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn raw_batch_get(
        ...
        let future = future_raw_batch_get(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.raw_batch_get.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn raw_scan(
        ...
        let future = future_raw_scan(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.raw_scan.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn raw_batch_scan(
        ...
        let future = future_raw_batch_scan(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.raw_batch_scan.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn raw_put(
        ...
        let future = future_raw_put(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.raw_put.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn raw_batch_put(
        ...
        let future = future_raw_batch_put(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.raw_batch_put.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn raw_delete(
        ...
        let future = future_raw_delete(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.raw_delete.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn raw_batch_delete(
        ...
        let future = future_raw_batch_delete(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.raw_batch_delete.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn raw_delete_range(
        ...
        let future = future_raw_delete_range(&self.storage, req)
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.raw_delete_range.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn unsafe_destroy_range(
        ...
        let future = AndThenWith::new(res, f.map_err(Error::from))
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.unsafe_destroy_range.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn coprocessor(&mut self, ctx: RpcContext<'_>, req: Request, sink: UnarySink<Response>) {
        ...
        let future = future_cop(&self.cop, req, Some(ctx.peer()))
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.coprocessor.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn coprocessor_stream(
        ...
        let future = sink
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.coprocessor_stream.observe_elapsed(instant);
                })
            })
            .map_err(Error::from)
            .map_err(move |e| {
    ...
    fn mvcc_get_by_key(
        ...
        let future = AndThenWith::new(res, f.map_err(Error::from))
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.mvcc_get_by_key.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn mvcc_get_by_start_ts(
        ...
        let future = AndThenWith::new(res, f.map_err(Error::from))
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.mvcc_get_by_start_ts.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn split_region(
        ...
        let future = future
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.split_region.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    fn read_index(
        ...
        let future = future
            ...
            .map(move |_| {
                GRPC_MSG_HISTOGRAM.may_flush(|m| {
                    m.read_index.observe_elapsed(instant);
                })
            })
            .map_err(move |e| {
                debug!("kv rpc failed";
    ...
    F: Future<Item = batch_commands_response::Response, Error = ()> + Send + 'static,
{
    let f = resp.and_then(move |resp| {
        if tx.send_and_notify((id, resp)).is_err() {
            ...
            GRPC_MSG_HISTOGRAM.may_flush(|m| {
                m.get(histogram_key).observe_elapsed(begin);
            });
            return Err(());
        }
        GRPC_MSG_HISTOGRAM.may_flush(|m| {
...
pub fn future_batch_get_command<E: Engine, L: LockManager>(
    ...
    storage.batch_get_command(commands).then(move |v| {
        ...
        GRPC_MSG_HISTOGRAM.may_flush(|m| {
            m.kv_batch_get_command.observe_elapsed(instant);
        });
        Ok(())
    })
}
...
pub fn future_raw_batch_get_command<E: Engine, L: LockManager>(
    ...
    storage.raw_batch_get_command(cf, commands).then(move |v| {
        ...
        GRPC_MSG_HISTOGRAM.may_flush(|m| {
            m.raw_batch_get_command.observe_elapsed(instant);
        });
        Ok(())
    })
}
