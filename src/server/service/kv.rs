...
use tracker::{
    set_tls_tracker_token, RequestInfo, RequestType, Tracker, GLOBAL_TRACKERS,
...
use crate::{
    ...
    server::{
        config::AtomicConfigs, gc_worker::GcWorker, load_statistics::ThreadLoadPool, metrics::*,
        snap::Task as SnapTask, Error, Proxy, Result as ServerResult,
    },
    storage::{
        errors::{
...
macro_rules! handle_request {
    ...
    ($fn_name: ident, $future_name: ident, $req_ty: ident, $resp_ty: ident, $time_detail: tt) => {
        fn $fn_name(&mut self, ctx: RpcContext<'_>, mut req: $req_ty, sink: UnarySink<$resp_ty>) {
            ...
            let resp = $future_name(&self.storage, req, &self.configs);
            let task = async move {
                let resp = resp.await?;
    ...
    for Service<T, E, L, F>
{
    ...
    fn raw_coprocessor(
        ...
        let source = req.mut_context().take_request_source();
        let begin_instant = Instant::now();
        let future = future_raw_coprocessor(&self.copr_v2, &self.storage, req, &self.configs);
        let task = async move {
    ...
    fn raft(
        ...
        let reject_messages_on_memory_ratio = self.configs.get_reject_messages_on_memory_ratio();
            ...
            match req.cmd {
                ...
                Some(batch_commands_request::request::Cmd::Get(mut req)) => {
                    if batcher.as_mut().map_or(false, |req_batch| {
                       ...
                       let begin_instant = Instant::now();
                       let source = req.mut_context().take_request_source();
                       let resp = future_get(storage, req, configs)
                ...
                Some(batch_commands_request::request::Cmd::RawGet(mut req)) => {
                    if batcher.as_mut().map_or(false, |req_batch| {
                       ...
                       let begin_instant = Instant::now();
                       let source = req.mut_context().take_request_source();
                       let resp = future_raw_get(storage, req, configs)
                ...
                $(Some(batch_commands_request::request::Cmd::$cmd(mut req)) => {
                    let begin_instant = Instant::now();
                    let source = req.mut_context().take_request_source();
                    let resp = $future_fn($($arg,)* req, configs)
...
macro_rules! init_tracker {
    ($configs:expr, $req:expr, $tp:expr, $ts:expr) => {{
        if $configs.get_track_request() {
            GLOBAL_TRACKERS.insert(Tracker::new(RequestInfo::new($req.get_context(), $tp, $ts)))
        } else {
            INVALID_TRACKER_TOKEN
        }
    }};
...
fn future_get<E: Engine, L: LockManager, F: KvFormat>(
    ...
    let tracker = init_tracker!(configs, &req, RequestType::KvGet, req.get_version());
...
fn future_scan<E: Engine, L: LockManager, F: KvFormat>(
    ...
    let tracker = init_tracker!(configs, &req, RequestType::KvScan, req.get_version());
    set_tls_tracker_token(tracker);
...
fn future_batch_get<E: Engine, L: LockManager, F: KvFormat>(
    ...
    let tracker = init_tracker!(configs, &req, RequestType::KvBatchGet, req.get_version());
    set_tls_tracker_token(tracker);
    let start = Instant::now();
...
) -> impl Future<Output = ServerResult<GcResponse>> {
    async {
...
macro_rules! txn_command_future {
    ($fn_name: ident, $req_ty: ident, $resp_ty: ident, ($req: ident) $prelude: stmt; ($v: ident, $resp: ident, $tracker: ident) { $else_branch: expr }) => {
        fn $fn_name<E: Engine, L: LockManager, F: KvFormat>(
            ...
            $prelude
            let $tracker = if configs.get_track_request() {
