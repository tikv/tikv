...
impl<T: RaftStoreRouter + 'static, E: Engine> Tikv for Service<T, E> {
            ...
            .map(move |mut v| {
                ...
                if v.response.get_header().has_error() {
                    ...
                    if admin_resp.get_splits().get_regions().len() != 2 {
                        ...
                        let mut regions = admin_resp.mut_splits().take_regions();
    ...
    fn read_index(
        ...
        cmd.set_requests(vec![inner_req]);
    ...
    fn batch_commands(
        ...
        let request_handler = stream.for_each(move |mut req| {
            ...
            let requests = req.take_requests();
    ...
    storage
        ...
        .then(|v| {
            ...
            if let Some(err) = extract_region_error(&v) {
                ...
                resp.set_pairs(extract_kv_pairs(v));
            }
            Ok(resp)
        })
}
...
fn future_prewrite<E: Engine>(
    ...
    AndThenWith::new(res, f.map_err(Error::from)).map(|v| {
        ...
        if let Some(err) = extract_region_error(&v) {
            ...
            resp.set_errors(extract_key_errors(v));
        }
        resp
    })
}
    ...
    storage
        ...
        .then(|v| {
            ...
            if let Some(err) = extract_region_error(&v) {
                ...
                resp.set_kvs(extract_kv_pairs(v));
            }
            Ok(resp)
        })
}
...
fn extract_mvcc_info(mvcc: storage::MvccInfo) -> MvccInfo {
    ...
    mvcc_info.set_writes(vw);
    mvcc_info.set_values(vv);
    mvcc_info
}
