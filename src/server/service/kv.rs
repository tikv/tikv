...
impl<T: RaftStoreRouter + 'static> tikvpb_grpc::Tikv for Service<T> {
    ...
    fn kv_batch_get(
        ...
        let timer = GRPC_MSG_HISTOGRAM_VEC.kv_batchget.start_coarse_timer();
        ...
        let future = self.storage
            ...
            .map(|_| timer.observe_duration())
            .map_err(move |e| {
                debug!("{} failed: {:?}", "kv_batchget", e);
                GRPC_MSG_FAIL_COUNTER.kv_batchget.inc();
            });
    ...
    fn coprocessor_stream(
        ...
        const LABEL: &str = "coprocessor_stream";
        let timer = GRPC_MSG_HISTOGRAM_VEC
            .coprocessor_stream
            .start_coarse_timer();
