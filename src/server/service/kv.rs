...
fn future_get<E: Engine, L: LockManager>(
    ...
    async move {
        ...
        if let Some(err) = extract_region_error(&v) {
            ...
            match v {
                Ok((val, statistics, perf_statistics_delta, latency_stats)) => {
                    ...
                    time_detail.set_wait_wall_time_ms(latency_stats.wait_wall_time_ms as i64);
                    time_detail.set_process_wall_time_ms(latency_stats.process_wall_time_ms as i64);
                    match val {
...
fn future_batch_get<E: Engine, L: LockManager>(
    ...
    async move {
        ...
        if let Some(err) = extract_region_error(&v) {
            ...
            match v {
                Ok((kv_res, statistics, perf_statistics_delta, latency_stats)) => {
