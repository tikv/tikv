...
pub mod test_gc_worker {
    ...
    impl Engine for MultiRocksEngine {
        ...
        fn async_snapshot(
            ...
            self.engines.lock().unwrap()[&region_id].async_snapshot(ctx, callback)
        }
    }
}
...
mod tests {
    ...
    fn multi_gc_engine_setup(
        ...
        (engine, ri_provider, gc_runner, vec![r1, r2, r3])
    }
    ...
    fn test_gc_for_multi_rocksdb() {
        ...
        let (engine, _ri_provider, mut gc_runner, regions) =
            multi_gc_engine_setup(dir.path(), store_id);
        ...
        for region_id in 1..=3 {
            ...
            for i in 10 * (region_id - 1)..10 * region_id {
                ...
                // Stale MVCC-PUTs will be cleaned in write CF's compaction filter.
                must_get_none_on_region(&region_engine, region_id, &k, 150);
    ...
    fn test_gc_keys_for_multi_rocksdb() {
        ...
        let (engine, ri_provider, mut gc_runner, _regions) =
            multi_gc_engine_setup(dir.path(), store_id);
        ...
        for region_id in 1..=3 {
            ...
            for i in 10 * (region_id - 1)..10 * region_id {
                ...
                if i % 2 == 0 {
                    ...
                    must_get_on_region(&region_engine, region_id, &k, 150, b"value");
                } else {
                    must_get_none_on_region(&region_engine, region_id, &k, 150);
                    assert!(
                        region_engine
