...
mod tests {
    ...
    fn test_with_properties(path: &str, region: &Region) {
        ...
        engine.flush();
        engine.compact();
        let props = get_mvcc_properties_and_check_gc(Arc::clone(&db), region.clone(), 10, false);
        ...
        let props = get_mvcc_properties_and_check_gc(Arc::clone(&db), region.clone(), 10, true);
        assert_eq!(props.min_ts, 1.into());
        ...
        let props = get_mvcc_properties_and_check_gc(Arc::clone(&db), region.clone(), 0, false);
