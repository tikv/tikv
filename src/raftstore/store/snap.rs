...
pub mod tests {
    ...
    fn test_snap_file(get_db: DBBuilder, db_opt: Option<DBOptions>) {
        ...
        let mut s4 = Snap::<RocksSnapshot>::new_for_applying(
            dst_dir.path(),
            &key,
            Arc::clone(&size_track),
            deleter,
    ...
    fn test_snap_corruption_on_size_or_checksum() {
        ...
        assert!(Snap::<RocksSnapshot>::new_for_sending(
            dir.path(),
            &key,
            Arc::clone(&size_track),
            deleter.clone()
    ...
    fn test_snap_mgr_v2() {
        ...
        .unwrap();
        let mut s = Snap::<RocksSnapshot>::new_for_sending(
            &path,
            &key1,
            Arc::clone(&size_track),
            deleter.clone(),
        ...
        let s3 = Snap::<RocksSnapshot>::new_for_building(
            &path,
            &key2,
            Arc::clone(&size_track),
            deleter.clone(),
