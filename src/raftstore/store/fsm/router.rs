    ...
    Cs: FsmScheduler<Fsm = C> + Clone,
{
    ...
    pub fn force_send(
        ...
        match self.send(addr, high_priority, msg) {
            Ok(()) => Ok(()),
...
mod tests {
    ...
    fn test_basic() {
        ...
        match router.force_send(1, false, unreachable()) {
            Err(SendError(_)) => (),
            Ok(_) => panic!("send should fail"),
        }
        match router.send(1, false, unreachable()) {
        ...
        router.force_send(1, false, noop()).unwrap();
        router.send(1, false, noop()).unwrap();
        ...
        let sent_cnt = (0..)
            .take_while(|_| router.send(1, false, counter_closure(&counter)).is_ok())
            .count();
        match router.send(1, false, counter_closure(&counter)) {
            Err(TrySendError::Full(_)) => {}
            Err(TrySendError::Disconnected(_)) => panic!("mailbox should still be connected."),
            Ok(_) => panic!("send should fail"),
        }
        router
            .force_send(1, false, counter_closure(&counter))
            .unwrap();
        tx.send(1).unwrap();
        // Flush.
        let (tx, rx) = mpsc::unbounded();
