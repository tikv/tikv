...
impl ApplyCallback {
    ...
    fn invoke_all(self, host: &CoprocessorHost, applied_index: Option<u64>) {
        ...
        host.post_apply_1(&self.region, applied_index);
    }
...
impl ApplyContext {
    ...
    pub fn write_to_db(&mut self) -> bool {
        ...
        for cbs in self.cbs.drain(..) {
            ...
            let applied_index = last_applied_index.get(&region_id).cloned();
...
impl ApplyDelegate {
    ...
    fn handle_raft_committed_entries(
        ...
        debug!(
            "{} handle_raft_committed_entries is called, committed: [{:?}, {:?})",
            apply_ctx.cur_region_id,
            committed_entries.first().map(|e| e.get_index()),
            committed_entries.last().map(|e| e.get_index() + 1),
        );
