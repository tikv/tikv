...
impl RaftBatchSystem {
    ...
    fn start_system<T: Transport + 'static, C: PdClient + 'static>(
        ...
        let mut readers: Vec<_> = (0..workers.local_readers.len())
            .map(|_| LocalReader::new(&builder, region_peers.iter().map(|pair| pair.1.get_peer())))
            .collect();
        ...
        for local_reader in workers.local_readers.iter_mut() {
            let timer = LocalReader::new_timer();
            box_try!(local_reader.start_with_timer(readers.pop().unwrap(), timer));
        }
