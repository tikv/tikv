...
impl<T: Transport, C: PdClient> Store<T, C> {
    ...
    pub fn maybe_create_peer(&mut self, region_id: u64, msg: &RaftMessage) -> Result<bool> {
            ...
            .next()
        {
            ...
            if enc_start_key(exist_region) < data_end_key(msg.get_end_key()) {
                ...
                let (msg_start, msg_end) = (
                    data_key(msg.get_start_key()),
                    data_end_key(msg.get_end_key()),
                );
                let (exist_start, exist_end) =
                    (enc_start_key(exist_region), enc_end_key(exist_region));
                // Make sure the range of region from msg is covered by exist regions.
                // If so, means that the region may generated by some kinds of split
                // and merge by catching logs. So it is no need to accept snapshot.
                if msg_start < exist_start && !self.is_range_covered(msg_start, exist_start) {
                    insert = true;
                }
                if exist_end < msg_end && !self.is_range_covered(exist_end, msg_end) {
                    insert = true;
                }
                if insert {
    ...
    // check whether the range is covered by exist regions.
    fn is_range_covered(&self, start: Vec<u8>, end: Vec<u8>) -> bool {
