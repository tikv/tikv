...
mod tests {
    ...
    use config::{DbConfig, TiKvConfig};
    use pd::{PdClient, PdFuture, RegionStat};
    use pd::errors::Result as PdResult;
    use server::{PdStoreAddrResolver, RaftClient, ServerRaftStoreRouter, ServerTransport};
    use server::resolve::new_resolver;
    use storage::types::Key as MvccKey;
    use util::rocksdb::{new_engine_opt, CompactedEvent};
    ...
    impl PdClient for MockPdClient {
        ...
        fn get_store(&self, _: u64) -> PdResult<metapb::Store> {
            unimplemented!();
        }
    ...
    type T = ServerTransport<ServerRaftStoreRouter, PdStoreAddrResolver>;
    type C = MockPdClient;
    fn new_test_raftstore(db: Arc<DB>) -> Store<T, C> {
        let cfg = TiKvConfig::default();
        let (sender, receiver) = mpsc::channel();
        let evloop = create_event_loop::<T, C>(&cfg.raft_store).unwrap();
        let sendch = SendCh::new(evloop.channel(), "raftstore");
        let router = ServerRaftStoreRouter::new(sendch.clone(), sender);

        let sec_mgr = Arc::new(SecurityManager::new(&cfg.security).unwrap());
        let pd_client = Arc::new(MockPdClient::default());
        let coprocessor = Arc::new(CoprocessorHost::new(cfg.coprocessor, sendch.clone()));
        let snap_mgr = SnapManager::new(db.path().to_owned(), None, None);
        let snap_worker = Worker::new("snapshot worker");
        ...
        let (_, resolver) = new_resolver(Arc::clone(&pd_client)).unwrap();
        ...
        Store {
            cfg: Rc::new(cfg.raft_store),
            store: metapb::Store::new(),
            kv_engine: Arc::clone(&db),
            raft_engine: Arc::clone(&db),
            sendch: sendch,
            significant_msg_receiver: receiver,
            region_peers: HashMap::default(),
            pending_raft_groups: HashSet::default(),
            split_check_worker: Worker::new("split check worker"),
            region_worker: Worker::new("region worker"),
            raftlog_gc_worker: Worker::new("raftlog gc worker"),
            compact_worker: Worker::new("compact worker"),
            ...
            trans: trans,
            pd_client: pd_client,
            coprocessor_host: coprocessor,
            snap_mgr: snap_mgr,
            raft_metrics: RaftMetrics::default(),
            entry_cache_metries: Rc::new(RefCell::new(CacheQueryStats::default())),
            pending_votes: RingQueue::with_capacity(PENDING_VOTES_CAP),
            tag: "raftstore".to_owned(),
            start_time: time::get_time(),
            is_busy: false,
            store_stat: StoreStat::default(),
        }
    ...
    fn new_mvcc_key(i: u8) -> Vec<u8> {
        MvccKey::from_raw(&[i]).append_ts(0).encoded().to_owned()
    }

    // Initialize db with `n` SST files
    fn init_db_with_sst_files<P: AsRef<Path>>(path: P, n: u8) -> DB {
        let cfg = DbConfig::default();
        let db_opts = cfg.build_opt();
        let mut cfs_opts = cfg.build_cf_opts();
        for opts in &mut cfs_opts {
            opts.set_level_zero_file_num_compaction_trigger(1);
        }

        let db = new_engine_opt(path.as_ref().to_str().unwrap(), db_opts, cfs_opts).unwrap();
    ...
    fn test_clear_stale_data() {
        let temp_dir = TempDir::new("tikv_test_clear_stale_data").unwrap();

        let n = 10;
        let db = Arc::new(init_db_with_sst_files(temp_dir.path(), n));

        let mut id = 1;
        let mut store = metapb::Store::new();
        store.set_id(id);

        // Make regions (1, 3, 5, ..., n), each region covers one SST file.
        let mut regions = Vec::new();
        for i in 0..n {
            if i % 2 == 0 {
                continue;
            }
            let mut p = metapb::Peer::new();
            id += 1;
            p.set_id(id);
            p.set_store_id(store.get_id());
            let mut r = metapb::Region::new();
            id += 1;
            r.set_id(id);
            r.mut_peers().push(p);
            r.set_start_key(new_mvcc_key(i));
            r.set_end_key(new_mvcc_key(i + 1));
            regions.push(r);
        }

        let mut s = new_test_raftstore(Arc::clone(&db));
        s.store = store;

        for r in &regions {
            let peer = Peer::create(&mut s, r).unwrap();
            s.region_peers.insert(r.get_id(), peer);
            s.region_ranges
                .insert(r.get_end_key().to_owned(), r.get_id());
        }
