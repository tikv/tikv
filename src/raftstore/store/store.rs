...
impl<T: Transport, C: PdClient> Store<T, C> {
    ...
    fn on_raft_message(&mut self, mut msg: RaftMessage) -> Result<()> {
        ...
        if self.need_reply_snapshot_failure(&msg) {
            self.reply_snapshot_failure(&msg);
            return Ok(());
        }
        ...
        if !self.region_peers.contains_key(&region_id) {
            let peer = try!(Peer::replicate(self, region_id, msg.get_region_epoch(), to_peer_id));
        ...
        if try!(self.is_snapshot_overlapped(&msg)) {
            self.reject_overlapped_snapshot(&msg);
            return Ok(());
        }
        ...
        if msg.get_message().get_msg_type() == raftpb::MessageType::MsgSnapStatus {
            // This can only happen when follower ignores the snapshot and responses
            // us a reject message.
            info!("follower {} rejects the snapshot generated by leader {} for region {}",
                  from_peer_id,
                  to_peer_id,
                  region_id);
            peer.raft_group.report_snapshot(from_peer_id, SnapshotStatus::Failure);
        } else {
    ...
    // If the peer meet another non heartbeat message, it will terminate rejecting snapshot state.
    // Return true means we will ignore the message and reply snapshot failure.
    fn need_reply_snapshot_failure(&mut self, msg: &RaftMessage) -> bool {
        let region_id = msg.get_region_id();
        if let Some(peer) = self.region_peers.get_mut(&region_id) {
            if peer.rejecting_snapshot &&
               msg.get_message().get_msg_type() == raftpb::MessageType::MsgHeartbeat {
                warn!("peer {} is in rejecting snapshot state for region {}, reply snapshot \
                       failure",
            ...
            // we receive another different message (not heartbeat), and we can think
    ...
    fn reply_snapshot_failure(&self, msg: &RaftMessage) {
        let region_id = msg.get_region_id();
        let from = msg.get_from_peer();
        let to = msg.get_to_peer();
        let epoch = msg.get_region_epoch();

        let mut resp = RaftMessage::new();
        resp.set_region_id(region_id);
        resp.set_from_peer(to.clone());
        resp.set_to_peer(from.clone());
        resp.set_region_epoch(epoch.clone());

        let mut m = raftpb::Message::new();
        m.set_msg_type(raftpb::MessageType::MsgSnapStatus);
        m.set_from(to.get_id());
        m.set_reject(true);

        resp.set_message(m);

        if let Err(e) = self.trans.rl().send(resp) {
            error!("send reject snapshot message for region {} failed {:?}",
                   region_id,
                   e);
        }
    }
