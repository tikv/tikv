...
mod tests {
    ...
    use engine::rocks;
    use engine::rocks::{RawCFOptions as ColumnFamilyOptions, Snapshot};
    use engine::DbEngines;
    use engine::{KvEngine, Mutable, Peekable};
    ...
    fn test_pending_applies() {
        ...
        for cf_name in engine.kv.cf_names() {
            let cf = rocks::util::get_cf_handle(engine.kv.as_ref(), cf_name).unwrap();
            for i in 0..6 {
                engine.kv.put_cf(cf_name, &[i], &[i]).unwrap();
                engine.kv.put_cf(cf_name, &[i + 1], &[i + 1]).unwrap();
                engine.kv.as_ref().flush_cf(cf, true).unwrap();
                // check level 0 files
                assert_eq!(
                    rocks::util::get_cf_num_files_at_level(engine.kv.as_ref(), cf, 0).unwrap(),
                    u64::from(i) + 1
                );
            }
        }
        ...
        let engines = DbEngines::new(engine.kv.clone(), engine.kv.clone(), shared_block_cache);
        ...
        let cf = rocks::util::get_cf_handle(engine.kv.as_ref(), CF_DEFAULT).unwrap();
        ...
        rocks::util::compact_files_in_range(engine.kv.as_ref(), None, None, None).unwrap();
        assert_eq!(
            rocks::util::get_cf_num_files_at_level(engine.kv.as_ref(), cf, 0).unwrap(),
            0
        );
