...
mod tests {
    ...
    fn put_data(engine: &DB, mut start_idx: u64, end_idx: u64, fill_short_value: bool) {
        ...
        let write_value = if fill_short_value {
            Write::new_unprotected(WriteType::Put, 0, Some(b"shortvalue".to_vec()))
        } else {
            Write::new_unprotected(WriteType::Put, 0, None)
        }
        .to_bytes();
    ...
    fn test_region_approximate_keys() {
        ...
        for &(key, vlen) in &cases {
            ...
            let write_v = Write::new_unprotected(WriteType::Put, 0, None).to_bytes();
