...
mod tests {
    ...
    fn test_mvcc_properties() {
        ...
        for &(key, ts, write_type, entry_type) in &cases {
            ...
            let v = Write::commit(write_type, ts, None).to_bytes();
    ...
    fn bench_mvcc_properties(b: &mut Bencher) {
        ...
        for i in 0..num_entries {
            ...
            let k = keys::data_key(k.as_encoded());
            let w = Write::commit(WriteType::Put, ts, Some(s.as_bytes().to_owned()));
    ...
    fn test_rows_properties() {
        ...
        for i in 0..num_rows {
            ...
            let v = Write::commit(WriteType::Put, 0, None).to_bytes();
