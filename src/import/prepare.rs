...
impl<Client: ImportClient> PrepareRangeJob<Client> {
                ...
                match new_region {
                    Some(new_region) => {
                        let new_leader = region
                            .leader
                            .and_then(|p| find_peer(&new_region, p.get_store_id()));
    ...
    fn split_region(&self, region: &RegionInfo) -> Result<RegionInfo> {
        ...
        // The SplitRegion API requires a raw key.
        let split_key = Key::from_encoded(self.range.get_end().to_owned()).raw()?;

        let mut split = SplitRegionRequest::new();
        split.set_context(ctx);
        split.set_split_key(split_key);
        ...
        match res {
            Ok(mut resp) => {
                ...
                let new_leader = find_peer(&new_region, store_id);
