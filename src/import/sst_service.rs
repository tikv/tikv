...
impl<E: Engine> ImportSstService<E> {
    ...
    async fn apply_imp(
        ...
        limiter: Limiter,
        max_raft_size: usize,
    ) -> StdResult<Option<Range>, ImportPbError> {
...
impl<E: Engine> ImportSst for ImportSstService<E> {
    async fn switch_mode(
        ...
        Ok(tonic::Response::new(SwitchModeResponse::default()))
    }
    ...
    async fn set_download_speed_limit(
        ...
        let speed_limit = request.get_ref().get_speed_limit();
        ...
        Ok(tonic::Response::new(
            SetDownloadSpeedLimitResponse::default(),
        ))
    }
    ...
    async fn download(
        ...
        let resp = match self.threads.spawn(handle_task).await {
            Ok(r) => r,
            Err(e) => {
                let mut resp = DownloadResponse::default();
                let mut import_err = ImportPbError::default();
                import_err.set_message(format!("failed to join download result: {:?}", e));
    ...
    async fn apply(
        ...
        let resp = match self.threads.spawn(handle_task).await {
            ...
            Err(e) => {
                let mut resp = ApplyResponse::default();
                let mut import_err = ImportPbError::default();
                import_err.set_message(format!("failed to join remove result: {:?}", e));
                resp.set_error(import_err);
                resp
            }
        };
    ...
    async fn clear_files(
        ...
        let mut resp = ClearResponse::default();
        let response = match self.threads.spawn(handle_task).await {
            Ok(r) => r,
            Err(e) => {
                let mut resp = ClearResponse::default();
                let mut import_err = ImportPbError::default();
