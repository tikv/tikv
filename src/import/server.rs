...
impl ImportServer {
    pub fn new(tikv: &TiKvConfig, client: Arc<RpcClient>) -> ImportServer {
        let cfg = &tikv.server;
        let addr = SocketAddr::from_str(&cfg.addr).unwrap();

        let importer = KVImporter::new(tikv.import.clone(), tikv.rocksdb.clone()).unwrap();
        let import_service = ImportKVService::new(tikv.import.clone(), client, Arc::new(importer));
