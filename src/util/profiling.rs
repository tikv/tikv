...
impl DumpFileNameGuard {
    ...
    unsafe fn get_mut_ptr(&mut self) -> *mut c_char {
        self.0.as_mut().map_or(0 as *mut _, |v| v.as_mut_ptr() as *mut c_char)
    }
}
...
pub fn profile_memory(file: Option<&str>, flag: &Arc<AtomicBool>, sleep_interval: Duration) -> Option<JoinHandle<()>> {
    ...
    unsafe {
        let mut first_file_name = DumpFileNameGuard::from_cstring(file.map(|f| get_file_name(f, "-fir")));
    ...
    let mut second_file_name = DumpFileNameGuard::from_cstring(file.map(|f| get_file_name(f, "-sec")));
    Some(Builder::new().name(thd_name!("memory_prof")).spawn(move || {
        ...
        thread::sleep(sleep_interval);
        unsafe {
            if let Err(e) = jemallocator::mallctl_set(PROFILE_DUMP, second_file_name.get_mut_ptr()) {
...
mod test {
    ...
    fn test_profiling_memory() {
        ...
        assert!(handle.is_none(), "no profile should be run when last profile has not finished.");
