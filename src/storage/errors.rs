...
use crate::storage::{
    kv::{self, Error as EngineError, ErrorInner as EngineErrorInner, Statistics},
...
pub fn extract_kv_pairs_and_statistics(res: Result<(Vec<Result<KvPair>>, Statistics)>) -> (Vec<kvrpcpb::KvPair>, Statistics) {
    match res {
        Ok((r, s)) => {
            let r = r.into_iter()
                .map(|r| match r {
