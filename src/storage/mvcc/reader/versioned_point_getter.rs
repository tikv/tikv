...
mod tests {
    ...
    fn new_point_getter<E: Engine>(engine: &mut E, ts: TimeStamp) -> VersionedPointGetter<E::Snap> {
    ...
    ) -> VersionedPointGetter<E::Snap> {
        let snapshot = engine.snapshot(Default::default()).unwrap();
        VersionedPointGetterBuilder::new(snapshot, ts)
            .isolation_level(iso_level)
            .build()
            .unwrap()
    }

    fn must_get_key<S: Snapshot>(point_getter: &mut VersionedPointGetter<S>, key: &[u8]) {
    ...
    fn must_met_newer_ts_data<E: Engine>(
        ...
        let mut point_getter = VersionedPointGetterBuilder::new(snapshot.clone(), ts)
            .isolation_level(IsolationLevel::Si)
        ...
        let mut point_getter = VersionedPointGetterBuilder::new(snapshot, ts)
            .isolation_level(IsolationLevel::Si)
            .check_has_newer_ts_data(false)
            .build()
            .unwrap();
    ...
    fn must_get_none<S: Snapshot>(point_getter: &mut VersionedPointGetter<S>, key: &[u8]) {
        assert!(point_getter.get(&Key::from_raw(key)).unwrap().is_none());
    }

    fn must_get_err<S: Snapshot>(point_getter: &mut VersionedPointGetter<S>, key: &[u8]) {
