...
impl<S: Snapshot> ForwardScanner<S> {
    pub fn new(
        builder: ScannerBuilder<S>,
        lock_cursor: Cursor<S::Iter>,
        write_cursor: Cursor<S::Iter>,
    ) -> Self {
        ForwardScanner {
            snapshot: builder.snapshot,
            fill_cache: builder.fill_cache,
            omit_value: builder.omit_value,
            isolation_level: builder.isolation_level,
            lower_bound: builder.lower_bound,
            upper_bound: builder.upper_bound,
            ts: builder.ts,
            lock_cursor,
            write_cursor,
            default_cursor: None,
...
mod tests {
    ...
    fn test_basic_scan_backward() {
        ...
        let mut scanner = match ScannerBuilder::new(snapshot, REVERSE_SEEK_BOUND, true)
            .range(None, Some(Key::from_raw(&[11 as u8])))
            .build()
            .unwrap()
        {
            Scanner::Backward(scanner) => scanner,
            _ => unreachable!(),
        };
    ...
    fn test_reverse_get_out_of_bound_1() {
        ...
        let snapshot = engine.snapshot(&Context::new()).unwrap();
        let mut scanner = match ScannerBuilder::new(snapshot, REVERSE_SEEK_BOUND * 2, true)
            .range(None, None)
            .build()
            .unwrap()
        {
    ...
    fn test_move_prev_user_key_out_of_bound_1() {
        ...
        let snapshot = engine.snapshot(&Context::new()).unwrap();
        let mut scanner = match ScannerBuilder::new(snapshot, 1, true)
            .range(None, None)
            .build()
            .unwrap()
        {
    ...
    fn test_move_prev_user_key_out_of_bound_3() {
        ...
        let snapshot = engine.snapshot(&Context::new()).unwrap();
        let mut scanner = match ScannerBuilder::new(snapshot, REVERSE_SEEK_BOUND + 1, true)
            .range(None, None)
            .build()
            .unwrap()
        {
    ...
    #[test]
    fn test_range_scan_back_forward() {
        let engine = TestEngineBuilder::new().build().unwrap();
        ...
        let mut scanner = match ScannerBuilder::new(snapshot.clone(), 10, true)
    ...
    fn test_many_tombstones() {
        ...
        // Call reverse scan
        let ts = 2;
        let mut scanner = match ScannerBuilder::new(snapshot, ts, true)
            .range(None, Some(k))
            .build()
            .unwrap()
        {
    ...
    fn test_get_out_of_bound() {
        ...
        let snapshot = engine.snapshot(&Context::new()).unwrap();
        let mut scanner = match ScannerBuilder::new(snapshot, 10, false)
            .range(None, None)
            .build()
            .unwrap()
        {
            Scanner::Forward(scanner) => scanner,
            _ => unreachable!(),
        };
    ...
    fn test_move_next_user_key_out_of_bound_1() {
        ...
        let snapshot = engine.snapshot(&Context::new()).unwrap();
        let mut scanner = match ScannerBuilder::new(snapshot, SEEK_BOUND * 2, false)
            .range(None, None)
            .build()
            .unwrap()
        {
    ...
    fn test_range_scan_forward() {
        ...
        // Test both bound specified.
        let mut scanner = match ScannerBuilder::new(snapshot.clone(), 10, false)
            .range(Some(Key::from_raw(&[3u8])), Some(Key::from_raw(&[5u8])))
            .build()
            .unwrap()
        {
