...
impl<S: Snapshot> ScanPolicy<S> for LatestKvPolicy {
    ...
    fn handle_write(
        ...
        let value: Option<Value> = loop {
            let write = Write::parse(cursors.write.value(&mut statistics.write))?;
...
impl LatestEntryPolicy {
    pub fn new(after_ts: u64, output_delete: bool) -> Self {
        LatestEntryPolicy {
            after_ts,
            output_delete,
        }
            ...
            match write.write_type {
                WriteType::Put => {
                    ...
                    let entry_default = if write.short_value.is_none() {
                        ...
                        let default_value = super::super::util::near_load_data_by_write(
                            default_cursor,
                            &current_user_key,
                            write,
                            statistics,
...
fn scan_latest_handle_lock<S: Snapshot, T>(
    ...
    let result = match cfg.isolation_level {
        IsolationLevel::SI => {
            ...
            super::super::util::check_lock(&current_user_key, cfg.ts, lock).map(|_| ())
        }
...
mod latest_entry_tests {
    ...
    fn test_output_delete_and_after_ts() {
        ...
        let check = |ts: u64, after_ts: u64, output_delete, expected: Vec<&TxnEntry>| {
            let snapshot = engine.snapshot(&Context::default()).unwrap();
            let mut scanner = ScannerBuilder::new(snapshot, ts, false)
                .range(None, None)
