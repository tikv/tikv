...
pub enum PessimisticLockRes {
    ...
    /// Checked whether the key exists while handling the `AcquirePessimisticLock` command. The
    /// i-th item is true if the i-th key in the `AcquirePessimisticLock` command exists.
    Existence(Vec<bool>),
    Empty,
}
