...
impl FnCall {
    ...
    pub fn greatest_int(&self, ctx: &mut EvalContext, row: &[Datum]) -> Result<Option<i64>> {
        let mut r = try_opt!(self.children[0].eval_int(ctx, row));
        for exp in &self.children[1..] {
            let v = try_opt!(exp.eval_int(ctx, row));
            r = max(r, v)
        }
        Ok(Some(r))
    }
    ...
    pub fn greatest_real(&self, ctx: &mut EvalContext, row: &[Datum]) -> Result<Option<f64>> {
        let mut r = try_opt!(self.children[0].eval_real(ctx, row));
        for exp in &self.children[1..] {
            let v = try_opt!(exp.eval_real(ctx, row));
            r = r.max(v)
        }
        Ok(Some(r))
    }
    ...
    pub fn greatest_decimal<'a, 'b: 'a>(
        ...
        let mut r = try_opt!(self.children[0].eval_decimal(ctx, row));
        for exp in &self.children[1..] {
            let v = try_opt!(exp.eval_decimal(ctx, row));
            r = max(r, v)
        }
        Ok(Some(r))
    }
    ...
    pub fn greatest_time<'a, 'b: 'a>(
        ...
        for exp in &self.children[1..] {
            let v = try_opt!(exp.eval_time(ctx, row));
            r = max(r, v)
        }
        Ok(Some(r))
    }
    ...
    pub fn greatest_string<'a, 'b: 'a>(
        ...
        for exp in &self.children[1..] {
            let v = try_opt!(exp.eval_string(ctx, row));
            r = max(r, v)
        }
        Ok(Some(r))
    }
    ...
    pub fn regexp(&self, ctx: &mut EvalContext, row: &[Datum]) -> Result<Option<i64>> {
        ...
        s.push_str(str::from_utf8(&pattern)?);
...
mod test {
    ...
    fn test_regexp() {
        ...
        for (target_str, pattern_str, exp) in cases {
            let target = datum_expr(Datum::Bytes(target_str.as_bytes().to_vec()));
            let pattern = datum_expr(Datum::Bytes(pattern_str.as_bytes().to_vec()));
            let op = fncall_expr(ScalarFuncSig::RegexpSig, &[target, pattern]);
            let op = Expression::build(&mut ctx, op).unwrap();
            let got = op.eval(&mut ctx, &[]).unwrap();
            let exp = Datum::from(exp);
    ...
    fn test_regexp_binary() {
        ...
        for (target_str, pattern_str, exp) in cases {
            let target = datum_expr(Datum::Bytes(target_str.as_bytes().to_vec()));
            let pattern = datum_expr(Datum::Bytes(pattern_str.as_bytes().to_vec()));
            let op = fncall_expr(ScalarFuncSig::RegexpBinarySig, &[target, pattern]);
            let op = Expression::build(&mut ctx, op).unwrap();
            let got = op.eval(&mut ctx, &[]).unwrap();
            let exp = Datum::from(exp);
