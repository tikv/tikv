...
mod test {
    ...
    use coprocessor::dag::expr::{EvalConfig, EvalContext, Expression, FLAG_OVERFLOW_AS_WARNING};
    use std::str::FromStr;
    use std::sync::Arc;
    ...
    fn test_bit_count() {
        ...
        let mut ctx =
            EvalContext::new(Arc::new(EvalConfig::new(FLAG_OVERFLOW_AS_WARNING).unwrap()));
        for (input, exp) in cases {
            let args = &[datum_expr(input)];
