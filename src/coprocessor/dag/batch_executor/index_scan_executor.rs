...
impl<S: Store> BatchIndexScanExecutor<S> {
    ...
    pub fn new(
        ...
        //
        // Note 3: Due to a bug in TiDB, it may send columns info with multiple handles, so we only
        // check the last column info to see if it is a pk handle.
        let decode_handle = columns_info.last().map_or(false, |ci| ci.get_pk_handle());
        let schema: Vec<_> = columns_info
            .iter()
            .map(|ci| super::scan_executor::field_type_from_column_info(&ci))
            .collect();
