...
impl<S: Store> BatchTableScanExecutor<S> {
    pub fn new(
        ...
        Ok(Self {
            ...
            ranges: RangesConsumer::new(key_ranges),
            scanner: None,
...
impl<S: Store> Executor for BatchTableScanExecutor<S> {
    ...
    fn next_batch(&mut self, expect_rows: usize) -> BatchExecuteResult {
        // Construct empty columns, with PK in decoded formast and the rest in raw format.
        let columns_len = self.context.columns_info.len();
        let mut columns = Vec::with_capacity(columns_len);
        for i in 0..columns_len {
            if i == self.handle_index {
                // For primary key, we construct a decoded `BatchColumn`.
...
mod tests {
    ...
    use super::*;
    use coprocessor::dag::executor::scanner::tests::{
