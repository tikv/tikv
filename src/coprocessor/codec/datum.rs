...
use super::mysql::{
    ...
    JsonDecoder, JsonEncoder, PathExpression, RoundMode, Time, DEFAULT_FSP, MAX_FSP,
};
...
pub trait DatumDecoder:
    JsonDecoder + MemComparableByteDecoder + CompactByteDecoder + DecimalDecoder
{
    ...
    fn decode_datum(&mut self) -> Result<Datum> {
        ...
        let datum = match flag {
            ...
            BYTES_FLAG => self.read_bytes().map(Datum::Bytes)?,
...
pub fn split_datum(buf: &[u8]) -> Result<(&[u8], &[u8])> {
    if buf.is_empty() {
    ...
    let pos = match buf[0] {
        ...
        BYTES_FLAG => MemComparableByteCodec::get_first_encoded_len(&buf[1..]),
...
mod tests {
    ...
    fn test_split_datum() {
        ...
        for case in table {
            ...
            for exp in &case {
                let (act, rem) = split_datum(buf).unwrap();
