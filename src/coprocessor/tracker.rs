...
impl<E: Engine> Tracker<E> {
    ...
    fn track(&mut self) {
        ...
        // Track MVCC read activity for load-based compaction prioritization
        let region_id = self.req_ctx.context.get_region_id();
        with_tls_tracker(|tracker| {
            ...
            if versions_scanned > 0 {
                ...
                if let Some(mvcc_tracker) = MVCC_READ_TRACKER.get() {
                    mvcc_tracker.record_read(region_id, versions_scanned);
                }
            }
        });
