...
impl<S: Snapshot> RowSampleBuilder<S> {
    ...
    async fn collect_column_stats(&mut self) -> Result<AnalyzeSamplingResult> {
        ...
        while !is_drained {
            ...
            // Don't let analyze bandwidth limit the quota limiter, this is already limited in rate
            // limiter.
            let quota_delay = {
    ...
    async fn collect_columns_stats(
        ...
        while !is_drained {
            ...
            if self.analyze_common_handle {
                // cur_val recording the current value's data and its counts when iterating index's
                // rows. Once we met a new value, the old value will be pushed into
                // the topn_heap to maintain the top-n information.
                let mut cur_val: (u32, Vec<u8>) = (0, vec![]);
                let mut topn_heap = BinaryHeap::new();
                for logical_row in &result.logical_rows {
                    let mut data = vec![];
...
/// `SampleCollector` will collect Samples and calculate the count, ndv and total size of an
/// attribute.
#[derive(Clone)]
