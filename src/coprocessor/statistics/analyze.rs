...
impl<S: Snapshot> AnalyzeContext<S> {
    ...
    async fn handle_full_sampling(builder: &mut RowSampleBuilder<S>, is_auto: bool) -> Result<Vec<u8>> {
    ...
    async fn handle_request(&mut self) -> Result<MemoryTraceGuard<Response>> {
        let ret = match self.req.get_tp() {
            ...
            AnalyzeType::TypeFullSampling => {
                ...
                // Use different quota limiter for auto and non-auto analyze request
                let quota_limiter =  self.quota_limiter.clone();

                let mut builder = RowSampleBuilder::new(col_req, storage, ranges, quota_limiter)?;
