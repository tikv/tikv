...
def template(name, query, dataSource, hide,
             regex=None, includeAll=False, allValue=None) -> Template:
    return Template(
...
def graph_legend(avg=False,
                 ...
                 rightSide=True,
                 sideWidth=None,
                 sortDesc=True) -> Legend:
    sort = "max" if max else "current"
    return Legend(avg=avg,
                  current=current,
                  max=max,
                  min=min,
                  show=show,
...
def expr_sum_rate(metrics: str,
    ...
    return expr_aggregate_function(
        metrics, 'sum', 'rate', range_selector, instant_selectors, by_labels,
        extra_expr)


def expr_sum_delta(metrics: str,
                   labels_selectors: list[str] = [],
                   range_selector: str = '$__rate_interval',
                   by_labels: list[str] = ["instance"],
                   default_labels_selectors: list[str] = [
    ...
    instant_selectors = default_labels_selectors + labels_selectors
    return expr_aggregate_function(
        metrics, 'sum', 'delta', range_selector, instant_selectors, by_labels,
        extra_expr)


def expr_sum(metrics: str,
             labels_selectors: list[str] = [],
             by_labels: list[str] = ["instance"],
             default_labels_selectors: list[str] = [
                 r'k8s_cluster="$k8s_cluster"',
    ...
    instant_selectors = default_labels_selectors + labels_selectors
    return expr_aggregate_function(
        metrics=metrics, agg_opeator='sum', labels_selectors=instant_selectors,
...
def expr_aggregate_function(metrics: str,
    ...
    instant_selectors = '{{{}}}'.format(
        ','.join(labels_selectors)) if labels_selectors else ''
...
def expr_simple(metrics: str,
    ...
    instant_selectors = '{{{}}}'.format(
        ','.join(default_labels_selectors)) if default_labels_selectors else ''
...
def expr_histogram_quantile(quantile: float, metrics: str,
                            labels_selectors: list[str] = [],
    ...
    layout.row([
        graph_panel(
            ...
            targets=[
                ...
                target(
                    expr=expr_histogram_quantile(
                        0.99, 'tikv_raftstore_request_wait_time_duration_secs_bucket'),
        ...
        graph_panel(
            ...
            targets=[
                target(
                    expr=expr_histogram_quantile(
                        0.99, 'tikv_storage_engine_async_request_duration_seconds_bucket', ['type="snapshot"']),
...
def Cluster() -> RowPanel:
    ...
    layout.row([
        graph_panel(
            ...
            targets=[
                target(
                    expr=expr_sum('tikv_store_size_bytes',
    ...
    layout.row([
        ...
        graph_panel(
            ...
            targets=[
                target(
                    expr=expr_sum_rate('tikv_engine_flow_bytes', labels_selectors=[
                        'type="wal_file_bytes"']),
                    legendFormat=r'{{instance}}-write',
                ),
                target(
                    expr=expr_sum_rate('tikv_engine_flow_bytes', labels_selectors=[
                        'type=~"bytes_read|iter_bytes_read"']),
    ...
    layout.row([
        graph_panel(
            ...
            targets=[
                target(
                    expr=expr_sum_rate('tikv_grpc_msg_duration_seconds_count', labels_selectors=[
                        'type!="kv_gc"']),
        ...
        graph_panel(
            ...
            targets=[
                target(
                    expr=expr_sum_rate('tikv_grpc_msg_fail_total', labels_selectors=[
                        'type!="kv_gc"']),
                    legendFormat=r'{{instance}}-grpc-msg-fail',
                ),
                target(
                    expr=expr_sum_delta('tikv_pd_heartbeat_message_total', labels_selectors=[
                        'type="noop"'], extra_expr='< 1'),
                    legendFormat=r'{{instance}}-pd-heartbeat',
                ),
                target(
                    expr=expr_sum_rate('tikv_critical_error_total',
    ...
    layout.row([
        graph_panel(
            ...
            targets=[
                target(
                    expr=expr_sum('tikv_raftstore_region_count', labels_selectors=[
                        'type="leader"']),
                    legendFormat=r'{{instance}}',
                ),
            ]),
