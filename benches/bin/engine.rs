...
fn create_engine() -> Box<Engine> {
    ...
    new_local_engine(TEMP_DIR, ALL_CFS).unwrap()
}
...
fn generate_row_data(rows: usize, ts_count: usize, value_length: usize) -> Vec<BenchDataItem> {
    ...
    for _ in 0..ts_count {
        for mut item in &mut result {
            item.ts.push(next_ts());
        }
    }
...
fn load_test_data(e: &Engine, data: &[BenchDataItem]) {
...
#[inline]
fn load_lock(snapshot: &Snapshot, key: &Key, statistics: &mut CFStatistics) {
...
#[inline]
fn seek_write(snapshot: &Snapshot, key: &Key, statistics: &mut CFStatistics) {
...
#[inline]
fn commit(e: &Engine, key: &Key, commit_ts: u64, statistics: &mut CFStatistics) {
    let snapshot = e.snapshot(&Context::new()).unwrap();
    load_lock(&*snapshot, key, statistics);
    // Dummy value in CF_WRITE
...
fn bench_get(e: &Engine, data: &[BenchDataItem]) -> BenchSamples {
    let snapshot = e.snapshot(&Context::new()).unwrap();
    let mut rng = rand::thread_rng();
...
fn bench_set(e: &Engine, data: &[BenchDataItem]) -> BenchSamples {
    //    let snapshot = e.snapshot(&Context::new()).unwrap();
    let mut rng = rand::thread_rng();
    let mut fake_statistics = CFStatistics::default();
...
fn bench_delete(e: &Engine, data: &[BenchDataItem]) -> BenchSamples {
    //    let snapshot = e.snapshot(&Context::new()).unwrap();
    let mut rng = rand::thread_rng();
    let mut fake_statistics = CFStatistics::default();
...
fn bench_single_gc(e: &Engine, data: &[BenchDataItem]) -> BenchSamples {
    let snapshot = e.snapshot(&Context::new()).unwrap();
    //    let mut rng = rand::thread_rng();
    let mut fake_statistics = CFStatistics::default();
