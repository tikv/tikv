...
fn bench_set<T: Simulator + TransportFilterable>(mut cluster: Cluster<T>,
                                                 value_size: usize)
                                                 -> BenchSamples {
...
fn bench_get<T: Simulator + TransportFilterable>(mut cluster: Cluster<T>) -> BenchSamples {
...
fn bench_seek<T: Simulator + TransportFilterable>(mut cluster: Cluster<T>) -> BenchSamples {
    let mut kvs = generate_random_kvs(DEFAULT_DATA_SIZE, 100, 128);
    prepare_cluster(&mut cluster, &kvs);
...
fn bench_delete<T: Simulator + TransportFilterable>(mut cluster: Cluster<T>) -> BenchSamples {
    let mut kvs = generate_random_kvs(DEFAULT_DATA_SIZE, 100, 128);
    prepare_cluster(&mut cluster, &kvs);
...
fn bench_raft_cluster<T, F>(factory: F, tag: &'static str)
    where T: Simulator + TransportFilterable,
