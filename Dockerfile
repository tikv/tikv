...
# Remove profiler example
RUN sed -i '/\[\[example\]\]/,+2d' ./components/profiler/Cargo.toml
...
RUN mkdir -p ./src/bin && \
    ...
    mkdir ./components/profiler/src && echo '' > ./components/profiler/src/lib.rs && \
    ...
    rm -rf ./target/release/.fingerprint/profiler-* && \
