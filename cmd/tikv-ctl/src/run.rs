...
use crate::{cmd::*, executor::*, fork_readonly_tikv, util::*};
...
pub fn flashback_whole_cluster(
    pd_client: &RpcClient,
    cfg: &TikvConfig,
    mgr: Arc<SecurityManager>,
    region_ids: Vec<u64>,
    ...
    start_key: Vec<u8>,
    end_key: Vec<u8>,
    debugger_client: Option<DebugClient>,
) {
    println!(
    ...
    block_on(runtime.spawn(async move {
        ...
        let debuggers = Mutex::new(stores
            ...
            .map(|s| {
                ...
                let mgr = Arc::clone(&mgr);
                let mut debug_executor = new_debug_executor(&cfg_inner, None, Some(&addr), mgr);
                if debugger_client.is_some() {
                    debug_executor = Box::new(debugger_client.clone().unwrap());
                }
