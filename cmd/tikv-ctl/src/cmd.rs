...
pub enum Cmd {
    ...
    CompactLogBackup {
        ...
        #[clap(
            ...
            help(
                "shard the compaction work by backup-stream store id. Determine which stores to \
                process based on the shard index and the hash of the store id. Format: \
                INDEX/TOTAL, where INDEX is 1-based (e.g. 1/3)."
            )
        )]
