...
## Summary
This document describes the TiKV-side pipeline that converts BR transactional
backup SSTs into Parquet files and optionally emits Iceberg table metadata (plus
a legacy
...
- Full incremental reconciliation into a single Iceberg table state (for example, deletes and merge semantics).
...
This section summarizes the performance plan for Iceberg/Parquet outputs and
the associated conversion-time overhead trade-offs.

### Priorities (highest impact first)
- Reduce small files: target larger Parquet objects (for example 128–512MB) to
  lower object-store request cost and Spark planning overhead.
- Improve pruning: add Iceberg partition specs and (optionally) sort order to
  reduce scanned data for common query patterns.
- Enrich Iceberg metadata: populate `DataFile` metrics (bounds/counts) so Spark
  can skip files more aggressively.
- Keep metadata scalable: shard manifests and avoid huge single manifests that
  slow down query planning.
- Improve type mapping: use Iceberg-friendly Parquet logical types (for example,
  decimal/timestamp) where applicable to improve predicate pushdown and
  readability.
- Support Iceberg evolution: generate proper multi-snapshot metadata (`vN.*`)
  instead of a single fixed metadata version.

### Overhead During Conversion (trade-offs)
- Compaction/merge (rewrite Parquet to larger files): higher read+write IO, CPU,
  and temporary storage; reduces long-term query cost and object-store requests.
- Sorting/clustering: CPU-heavy and may spill to disk; improves compression and
  min/max pruning at the expense of conversion time.
- Bloom filters: extra CPU and file size; improves equality-filter and point
  lookup performance.
- Rich metrics collection: low overhead when derived from Parquet footers;
  higher overhead if computed during write with large in-memory state.
- Compression codec choice: `snappy/lz4` favors conversion speed; `zstd/gzip`
  favors storage and network cost at the expense of CPU.
- Iceberg evolution: minimal runtime overhead per export, but more complexity in
  metadata writes and atomicity requirements.
...
- `--bloom-filter` enable Parquet bloom filters for INT/UTF8 columns (default: false)
...
## Incremental Backups
BR `backupmeta` uses `start_version` and `end_version` to describe a backup
interval:
- `start_version = 0`: full backup
- `start_version > 0`: incremental backup

The exporter accepts both full and incremental `backupmeta` inputs. For
incremental backups, the output should be treated as an **append-only delta**
containing row images present in the backup SSTs.

Important limitations for incremental exports:
- Delete semantics are not currently represented in Iceberg (for example, via
  equality/position delete files), so downstream consumers must plan for
  reconciliation/compaction if they need a fully correct table state.
- Each export run emits a single Iceberg snapshot (derived from `end_version`);
  multi-snapshot Iceberg versioning and incremental table evolution is future
  work.
...
- `manifest-{snapshot_id}.avro` (manifest, or sharded as `manifest-{snapshot_id}-00000.avro`, ...)
...
- Bloom filters are optional (`--bloom-filter`):
  - Enabled for INT/UTF8 columns (excluding `_tidb_table_id`) because these are the most common
    equality predicate columns (for example, handles and primary keys).
  - Uses Parquet bloom filters with `ndv = row_group_size` as a heuristic; `fpp` uses the Parquet
    crate default (0.05). This trades extra CPU + file size for faster point/range lookup pruning.
...
The generated manifests include Iceberg `DataFile` metrics (counts and bounds)
to improve file pruning during query planning.
...
- Incremental backups are accepted, but require downstream reconciliation if a
  fully correct table state (including deletes) is required.
...
- Integration tests cover row-group sizing, bloom filter emission, table filters, and resume behavior.
- Optional Spark Iceberg readback (requires `spark-sql` + Iceberg Spark runtime):
  - `SPARK_SQL=$(command -v spark-sql) ICEBERG_SPARK_RUNTIME_JAR=/path/to/iceberg-spark-runtime.jar cargo test -p br_parquet_exporter --test export -- --ignored integration_spark_sql_readback --nocapture`
- Manual smoke test (local TiUP, saved under `/Users/brian.w/projects/testdata`):
  ...
  - Bloom filter case:
    - `tiup br backup full --pd 127.0.0.1:2379 --storage local:///Users/brian.w/projects/testdata/br-backup-smoke-bloom --filter 'parquet_smoke.all_types'`
    - `BASE64_INPUT=$(tiup br operator base64ify -s "local:///Users/brian.w/projects/testdata/br-backup-smoke-bloom")`
    - `BASE64_OUTPUT=$(tiup br operator base64ify -s "local:///Users/brian.w/projects/testdata/parquet-export-smoke-bloom")`
    - `tikv-ctl br-parquet-export --input-storage-base64 "$BASE64_INPUT" --output-storage-base64 "$BASE64_OUTPUT" --filter 'parquet_smoke.all_types' --bloom-filter`
    - Validate bloom filter metadata:
      - `parquet-tools meta /Users/brian.w/projects/testdata/parquet-export-smoke-bloom/parquet/parquet_smoke/all_types/*.parquet | rg -n 'bloom filter offset|bloom filter length'`
...
- Iceberg metadata versioning (multiple snapshots, `vN.metadata.json` roll-forward, proper `version-hint.text`).
- Optional partition spec and sort order configuration for Iceberg tables.
- Compaction/optimization stage to merge small Parquet files into larger targets.
- Rich Iceberg `DataFile` metrics (bounds/counts) derived from Parquet metadata.
- Incremental reconciliation via Iceberg delete files or copy-on-write rewrite.
