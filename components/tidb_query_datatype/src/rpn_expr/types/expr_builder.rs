...
impl RpnExpressionBuilder {
    ...
    pub fn check_expr_tree_supported(c: &Expr) -> Result<()> {
        ...
        match c.get_tp() {
            ...
            ExprType::MysqlJson => {}
            ExprType::ColumnRef => {}
            _ => return Err(_other_err!("Blacklist expression type {:?}", c.get_tp())),
        }
    ...
    pub fn is_expr_eval_to_scalar(c: &Expr) -> Result<bool> {
        match c.get_tp() {
            ...
            _ => Err(_other_err!("Unsupported expression type {:?}", c.get_tp())),
        }
    }
...
fn handle_node_column_ref(
    ...
    let offset = tree_node
        .get_val()
        .read_i64()
        .map_err(|_| _other_err!("Unable to decode column reference offset from the request"))?
        as usize;
    if offset >= max_columns {
        return Err(_other_err!(
...
fn extract_scalar_value_int64(val: Vec<u8>) -> Result<ScalarValue> {
    let value = val
        .as_slice()
        .read_i64()
        .map_err(|_| _other_err!("Unable to decode int64 from the request"))?;
...
fn extract_scalar_value_uint64(val: Vec<u8>) -> Result<ScalarValue> {
    let value = val
        .as_slice()
        .read_u64()
        .map_err(|_| _other_err!("Unable to decode uint64 from the request"))?;
...
fn extract_scalar_value_float(val: Vec<u8>) -> Result<ScalarValue> {
    let value = val
        .as_slice()
        .read_f64()
        .map_err(|_| _other_err!("Unable to decode float from the request"))?;
...
fn extract_scalar_value_date_time(
    ...
    let v = val
        .as_slice()
        .read_u64()
        .map_err(|_| _other_err!("Unable to decode date time from the request"))?;
...
fn extract_scalar_value_duration(val: Vec<u8>) -> Result<ScalarValue> {
    let n = val
        .as_slice()
        .read_i64()
        .map_err(|_| _other_err!("Unable to decode duration from the request"))?;
...
fn extract_scalar_value_decimal(val: Vec<u8>) -> Result<ScalarValue> {
    ...
    let value = val
        .as_slice()
        .read_decimal()
        .map_err(|_| _other_err!("Unable to decode decimal from the request"))?;
...
fn extract_scalar_value_json(val: Vec<u8>) -> Result<ScalarValue> {
    let value = val
        .as_slice()
        .read_json()
        .map_err(|_| _other_err!("Unable to decode json from the request"))?;
