...
///
/// Each iteration goes through `decode_one_with_desc`, so a buffer that
/// happens to contain descending-order encoded columns (pingcap/tidb#2519)
/// is decoded transparently — the helper falls through to the ascending
/// path when the leading flag is not a DESC marker, so ASC-only callers
...
mod tests {
    ...
    fn test_split_datum_handles_desc_flags() {
        ...
        for case in cases {
            ...
            assert_eq!(head, bs.as_slice(), "head must cover the whole datum for {:?}", case);
