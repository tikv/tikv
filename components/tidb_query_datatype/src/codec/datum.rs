...
///
/// Intended for stored row-value streams (e.g. `decode_row` in `table.rs`)
/// where the encoder contract guarantees no range-bound sentinels appear.
/// Each iteration goes through `decode_one_with_desc`, so a stream that
/// contains descending-order encoded columns (pingcap/tidb#2519) is decoded
/// transparently — the helper falls through to the ascending path when the
/// leading flag is not a DESC marker, so ASC-only streams see no behaviour
/// change.
///
/// Do not pass range-bound buffers (which can legitimately contain
/// `Datum::Max`/`MAX_FLAG`) here: a 9+ byte buffer starting with `MAX_FLAG`
/// would be reinterpreted as a DESC float because `MAX_FLAG ==
/// FLOAT_DESC_FLAG`. Use `read_datum` directly for such inputs. See the comment
/// above `FLOAT_DESC_FLAG` for the disambiguation invariant.
pub fn decode<'a>(data: &mut BytesSlice<'a>) -> Result<Vec<Datum>> {
    let mut res = vec![];
    while !data.is_empty() {
        let (v, rem) = decode_one_with_desc(*data)?;
        res.push(v);
        *data = rem;
    }
    Ok(res)
    ...
    let pos = match buf[0] {
        ...
        COMPACT_BYTES_DESC_FLAG => {
            ...
            match NumberCodec::try_decode_var_i64(&inv[..prefix_len]) {
                // A negative length is nonsensical for a byte payload; treat
                // it as malformed and clamp to the tail. `usize::try_from` +
                // `checked_add` keep this branch panic-free in debug builds
                // (where unsigned overflow checks would otherwise trigger).
                Ok((value, decoded_n)) if value >= 0 => usize::try_from(value)
                    .ok()
                    .and_then(|n| n.checked_add(decoded_n))
                    .map(|n| n.min(tail.len()))
                    .unwrap_or(tail.len()),
                _ => tail.len(),
            }
        }
...
mod tests {
    ...
    /// `decode()` must transparently handle a buffer that mixes ASC and DESC
    /// encoded datums in arbitrary order. This exercises the
    /// `decode_one_with_desc` fall-through path inside the loop body.
    #[test]
    fn test_decode_handles_mixed_asc_and_desc_stream() {
        let mut ctx = EvalContext::default();
        ...
        let got = decode(&mut slice).unwrap();
    ...
    /// of `decode` / `decode_one_with_desc` must not pass range-bound buffers
    /// where `Datum::Max` is a legal sentinel — those must go through
    /// `read_datum` directly. The complement of all-zero DESC-encoded float
