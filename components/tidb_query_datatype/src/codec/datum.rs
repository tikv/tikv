...
// computing per-column lengths; `read_datum` and `decode` are ASC-only by
// design and the DESC-aware entrypoint `decode_one_with_desc` routes the
// bytes through `un_invert_if_desc` before dispatching to them. The
...
//  * `decode` / `decode_one_with_desc` are invoked on stored row data, which
//    never contains `Datum::Max`.
//  * The range-sentinel path encodes `Datum::Max` as a one-byte buffer; if such
//    a buffer reached `decode_one_with_desc`, `split_datum` would return "too
//    short" because a DESC-float layout requires 9 bytes total.
...
pub const INT_DESC_FLAG: u8 = !INT_FLAG;
pub const UINT_DESC_FLAG: u8 = !UINT_FLAG;
pub const FLOAT_DESC_FLAG: u8 = !FLOAT_FLAG;
pub const DECIMAL_DESC_FLAG: u8 = !DECIMAL_FLAG;
pub const DURATION_DESC_FLAG: u8 = !DURATION_FLAG;

/// Reports whether `b` is the leading flag byte of a descending-order-encoded
/// datum — i.e. the bitwise complement of an ascending flag that `split_datum`
/// and `read_datum` know how to decode.
///
/// Intentionally excludes `!JSON_FLAG` and `!VECTOR_FLOAT32_FLAG`: TiDB does
/// not emit DESC-complemented JSON or vector datums. DDL rejects DESC on
/// columnar/vector indexes (`pkg/ddl/index.go`), JSON columns are not
/// directly indexable (`pkg/ddl/index.go`), and `EncodeKeyWithDesc` hard-
/// rejects `KindMysqlJSON` and `KindVectorFloat32` (`pkg/util/codec/codec.go`).
/// This helper mirrors TiDB's `IsDescFlag` (`pkg/util/codec/codec.go`).
pub fn is_desc_flag(b: u8) -> bool {
    matches!(
        b,
        NIL_DESC_FLAG
            | BYTES_DESC_FLAG
    ...
    DecimalDecoder + JsonDecoder + VectorFloat32Decoder + CompactByteDecoder + MemComparableByteDecoder
{
    ...
    ///
    /// This trait method handles ascending-order encoded datums only. To
    /// decode a key that may contain descending-order columns
    /// (pingcap/tidb#2519), use the slice helper `decode_one_with_desc` which
    /// can peek the leading flag, complement a scratch buffer, and then call
    /// back into this trait — the BufferReader interface can't rewind for
    /// variable-length DESC types so we keep that detection out of band.
...
/// `un_invert_if_desc` complements a descending-order index column
...
/// owned buffer; otherwise the input is borrowed unchanged. Use it when a
/// whole column slice needs the ASC form before a type-specific decoder
/// (`read_datum`, `decode_int_datum`, ...); hot paths that write into an
/// existing buffer complement in place instead (see
/// `BufferVec::push_inverted`).
...
pub fn decode_one_with_desc(buf: &[u8]) -> Result<(Datum, &[u8])> {
    if buf.is_empty() {
        return Err(box_err!("buf is empty"));
    }
    if !is_desc_flag(buf[0]) {
        let mut slice = buf;
    ...
    debug_assert!(
        slice.is_empty(),
        "split_datum length and read_datum consumption disagree: {} byte(s) left",
        slice.len(),
    );
    Ok((datum, tail))
}

/// `decode` decodes all datums from a byte slice generated by tidb.
///
/// Ascending-encoded datums only: row-value streams (e.g. `decode_row` in
/// `table.rs`) never contain descending-order columns, and keeping this
/// entrypoint ASC-only avoids the `MAX_FLAG == FLOAT_DESC_FLAG` ambiguity
/// for callers whose buffers may legitimately contain range-bound sentinels.
/// Index keys that may contain descending-order columns (pingcap/tidb#2519)
/// must be walked with `decode_one_with_desc` instead (see
/// `table::decode_index_key`).
...
///
/// `desc` controls how a `BYTES_FLAG` body is sized (the encoder distinguishes
/// ASC and DESC bytes by the marker pattern within the body, not by the flag).
/// Index columns produced by TiDB's `EncodeKeyWithDesc` (pingcap/tidb#2519)
/// instead carry a bitwise-complemented leading flag and a complemented body;
/// we auto-detect those and compute lengths from the un-complemented form,
/// so the caller does not need per-column desc metadata.
pub fn split_datum(buf: &[u8], desc: bool) -> Result<(&[u8], &[u8])> {
    ...
    let pos = match buf[0] {
        ...
        // the sizers peek only the few complemented marker/prefix bytes they
        // need — allocating + flipping the entire tail (which can be
        // megabytes for a long BYTES column) would turn a constant-time
        // length probe into an O(n) copy on a hot index-walk path.
        NIL_DESC_FLAG => 0,
        INT_DESC_FLAG | DURATION_DESC_FLAG => number::I64_SIZE,
        UINT_DESC_FLAG => number::U64_SIZE,
        FLOAT_DESC_FLAG => number::F64_SIZE,
        BYTES_DESC_FLAG => MemComparableByteCodec::get_first_encoded_len_desc(&buf[1..]),
        COMPACT_BYTES_DESC_FLAG => CompactByteCodec::get_first_encoded_len_desc(&buf[1..]),
        DECIMAL_DESC_FLAG => {
...
mod tests {
    ...
    /// Mirror TiDB's `EncodeKeyWithDesc` for descending-order index columns
    /// (pingcap/tidb#2519): encode each datum normally and bitwise-complement
    /// every byte. Used by tests below to exercise the decoder without
    /// pulling in the Go encoder.
    fn encode_one_desc(ctx: &mut EvalContext, d: &Datum) -> Vec<u8> {
        let mut bs = encode_key(ctx, from_ref(d)).unwrap();
        for b in &mut bs {
            *b = !*b;
        }
        bs
    ...
    #[test]
    fn test_split_datum_handles_desc_flags() {
        let cases = vec![
            Datum::I64(-5),
            Datum::I64(0),
            Datum::I64(42),
            Datum::U64(0),
            Datum::U64(99),
            // Pick a value that isn't close to a math constant — clippy's
            // approx_constant lint flags 3.14 / 2.71828 as PI / E approximates.
            Datum::F64(7.5),
            Datum::Null,
        ...
        let mut ctx = EvalContext::default();
        for case in cases {
            let bs = encode_one_desc(&mut ctx, &case);
            let (head, rem) = split_datum(&bs, false)
                .unwrap_or_else(|e| panic!("split_datum failed for {:?}: {}", case, e));
            assert_eq!(
                head,
                bs.as_slice(),
                "head must cover the whole datum for {:?}",
                case
            );
            assert!(rem.is_empty(), "no trailing bytes expected for {:?}", case);
        }
    }

    #[test]
    fn test_decode_one_with_desc_round_trip() {
        let mut ctx = EvalContext::default();
        let cases = vec![
            Datum::I64(-100),
            Datum::I64(1 << 30),
            Datum::U64(1234567890),
            Datum::F64(7.5),
            Datum::Null,
            Datum::Bytes(b"hello, world".to_vec()),
            // Duration goes through DURATION_DESC_FLAG (8-byte payload).
            Datum::Dur(Duration::from_millis(2_345_678, MAX_FSP).unwrap()),
            Datum::Dur(Duration::from_millis(-2_345_678, MAX_FSP).unwrap()),
            // Decimal goes through DECIMAL_DESC_FLAG; covers the prefix-only
            // optimisation in `split_datum` against full `read_datum`
            // consumption (the debug_assert in `decode_one_with_desc` fires
            // if the two ever drift).
            Datum::Dec(Decimal::from(1)),
            Datum::Dec("1234567890.12345678901".parse().unwrap()),
            Datum::Dec("-9876543210.0987654321".parse().unwrap()),
        ];
        for case in cases {
            let bs = encode_one_desc(&mut ctx, &case);
            let (got, rem) = decode_one_with_desc(&bs)
                .unwrap_or_else(|e| panic!("decode_one_with_desc failed for {:?}: {}", case, e));
            assert!(rem.is_empty(), "no trailing bytes expected for {:?}", case);
            assert_eq!(got, case, "round trip mismatch for {:?}", case);
        }
    }

    /// Walking a buffer that mixes ASC and DESC encoded datums in arbitrary
    /// order with `decode_one_with_desc` must yield the logical values. This
    /// exercises the fall-through path between consecutive columns the same
    /// way `table::decode_index_key` walks an index key.
    #[test]
    fn test_decode_one_with_desc_handles_mixed_asc_and_desc_stream() {
        let mut ctx = EvalContext::default();
        let asc_int = encode_key(&mut ctx, &[Datum::I64(42)]).unwrap();
        let desc_str = encode_one_desc(&mut ctx, &Datum::Bytes(b"zzz".to_vec()));
        let asc_str = encode_key(&mut ctx, &[Datum::Bytes(b"abc".to_vec())]).unwrap();
        let desc_int = encode_one_desc(&mut ctx, &Datum::I64(-7));

        let mut buf = Vec::new();
        buf.extend_from_slice(&asc_int);
        buf.extend_from_slice(&desc_str);
        buf.extend_from_slice(&asc_str);
        buf.extend_from_slice(&desc_int);

        let mut slice = buf.as_slice();
        let mut got = Vec::new();
        while !slice.is_empty() {
            let (v, rem) = decode_one_with_desc(slice).unwrap();
            got.push(v);
            slice = rem;
        }
        assert_eq!(
    ...
    #[test]
    fn test_decode_one_with_desc_falls_through_to_asc() {
        // ASC-encoded datums must decode unchanged through the same helper.
        let mut ctx = EvalContext::default();
        let asc = encode_key(&mut ctx, &[Datum::I64(7), Datum::Bytes(b"abc".to_vec())]).unwrap();
        let (d0, rem) = decode_one_with_desc(&asc).unwrap();
        assert_eq!(d0, Datum::I64(7));
    ...
    /// A malformed `COMPACT_BYTES_DESC_FLAG` payload whose varint length
    /// decodes negative must not panic in debug builds. The hardened length
    /// arithmetic rejects `value < 0` and falls back to clamping to
    /// `tail.len()`. Before the fix, `(value as usize + decoded_n)` would
    /// panic on unsigned overflow in debug builds.
    #[test]
    fn test_split_datum_malformed_compact_bytes_desc_does_not_panic() {
        // varint zig-zag of -1 is the single byte 0x01; bitwise complement
        // is 0xFE. `split_datum` re-inverts the prefix back to 0x01 and
        // decodes it as i64 = -1.
        let buf = [COMPACT_BYTES_DESC_FLAG, !0x01u8];
        // Must not panic. Either Ok or Err is acceptable here — the
        // property under test is panic-freedom on malformed input.
        let _ = split_datum(&buf, false);
    }

    /// A 9-byte buffer starting with `MAX_FLAG` satisfies the length predicate
    /// for a DESC float and is therefore reinterpreted as one by
    /// `decode_one_with_desc`. This locks in the documented contract: callers
    /// of `decode_one_with_desc` must not pass range-bound buffers
    /// where `Datum::Max` is a legal sentinel — those must go through
    /// `read_datum` / `decode` directly. The complement of all-zero DESC-encoded float
    /// bytes is `f64::from_bits(u64::MAX)` (a quiet NaN); we just assert that
    /// a `Datum::F64` is produced rather than an error.
    #[test]
    fn test_decode_one_with_desc_reinterprets_max_flag_with_trailing_bytes() {
        let buf = [MAX_FLAG, 0, 0, 0, 0, 0, 0, 0, 0];
        let (d, rem) = decode_one_with_desc(&buf)
            .expect("[MAX_FLAG, 0×8] currently decodes as DESC float by design");
        assert!(rem.is_empty());
        assert!(matches!(d, Datum::F64(_)));
    }
