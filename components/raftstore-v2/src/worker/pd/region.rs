    ...
    T: PdClient + 'static,
{
    pub fn handle_region_heartbeat(&mut self, task: RegionHeartbeatTask) {
        ...
        let cpu_usage = {
            ...
            let interval_second = unix_secs_now.into_inner().saturating_sub(last_report_ts.into_inner());
    ...
    pub fn handle_report_region_buckets(&mut self, delta_buckets: BucketStat) {
        ...
        let interval_second = now.into_inner().saturating_sub(last_report_ts.into_inner());
