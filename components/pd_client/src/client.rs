...
use super::{
    ...
    BucketStat, Config, Error, FeatureGate, PdClientCommon, PdClientExt, PdFuture, RegionInfo,
...
impl PdClientCommon for Arc<RpcClient> {
...
impl PdClientExt for Arc<RpcClient> {
    fn region_heartbeat(
        &mut self,
        term: u64,
        region: metapb::Region,
    ...
    fn handle_region_heartbeat_response<F>(&mut self, _: u64, f: F) -> PdFuture<()>
    where
    ...
    fn handle_reconnect<F: Fn() + Sync + Send + 'static>(&mut self, f: F) {
...
impl crate::PdClient for Arc<RpcClient> {}
