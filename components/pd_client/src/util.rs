...
pub fn merge_bucket_stats<C: AsRef<[u8]>, I: AsRef<[u8]>>(
    ...
    for new_idx in 0..(incoming.len() - 1) {
        ...
        for cur_idx in start_idx..end_idx {
            member_add!(cur_stats, cur_idx, delta_stats, new_idx, read_bytes);
            member_add!(cur_stats, cur_idx, delta_stats, new_idx, write_bytes);

            member_add!(cur_stats, cur_idx, delta_stats, new_idx, read_qps);
            member_add!(cur_stats, cur_idx, delta_stats, new_idx, write_qps);

            member_add!(cur_stats, cur_idx, delta_stats, new_idx, read_keys);
            member_add!(cur_stats, cur_idx, delta_stats, new_idx, write_keys);
        }
    }
}
...
mod test {
    ...
    fn test_merge_bucket_stats() {
        ...
        for (current, incoming, expected) in cases {
            let cur_keys = unsafe { ::std::mem::transmute::<&[&[u8; 2]], &[&[u8]]>(&current.0) };
            let incoming_keys =
                unsafe { ::std::mem::transmute::<&[&[u8; 2]], &[&[u8]]>(&incoming.0) };
