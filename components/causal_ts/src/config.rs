...
pub struct Config {
    ...
    ///
    /// Default is 100.
    /// One TSO is required for every batch of Raft put messages, so by default 1K tso/s should be
    /// enough. Benchmark showed that with a 8.6w raw_put per second, the TSO requirement is
    /// 600 per second.
