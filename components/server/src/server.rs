...
static mut procStats: ProcessStat = ProcessStat::cur_proc_stat().unwrap();
        ...
        self.background_worker.spawn_interval_task(
            ...
            move || unsafe {
                ...
                if cpu_usage <= 0_f64 {
                    ...
                    let old_quota = self.quota_limiter.background_cputime_limiter();

                    if cpu_util >= SYSTEM_BUSY_THRESHOLD {
                        self.quota_limiter.set_background_cpu_time_limit(
                            1_000
                                * cmp::max(
                                (self.quota_limiter.background_cputime_limiter() / 1_000_000.0) as usize - 1,
                        ...
                        self.quota_limiter.set_background_cpu_time_limit(
                            1_000
                                * cmp::min(
                                (self.quota_limiter.background_cputime_limiter() / 1_000_000.0) as usize + 1,
                                (SysQuota::cpu_cores_quota() * BACKGROUND_REQUEST_CORE_MAX_RATIO) as usize,
                                ),
                        );
                    }

                    if old_quota != self.quota_limiter.background_cputime_limiter() {
                            debug!("cpu_time_limiter tuned for backend request";
                            "cpu_util" => ?cpu_util,
                            "new quota" => ?self.quota_limiter.background_cputime_limiter() / 1_000.0);
                    }
