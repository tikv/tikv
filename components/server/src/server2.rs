...
use tikv_util::{
    ...
    thread_name_prefix::{
        ...
        DEBUGGER_THREAD_PREFIX, GRPC_SERVER_THREAD_PREFIX,
        PD_WORKER_THREAD_PREFIX, RESOLVED_TS_WORKER_THREAD_PREFIX,
        SST_RECOVERY_THREAD_PREFIX, UNIFIED_READ_POOL_THREAD_PREFIX,
    },
    ...
    ER: RaftEngine,
{
    ...
    fn init_servers<F: KvFormat>(&mut self) -> Arc<VersionTrack<ServerConfig>> {
        ...
        let check_leader_worker =
            Box::new(WorkerBuilder::new(CHECK_LEADER_THREAD_PREFIX).thread_count(1).create());
        // Create check leader runer.
        let check_leader_runner = CheckLeaderRunner::new(
            self.router.as_ref().unwrap().store_meta().clone(),
            self.coprocessor_host.clone().unwrap(),
        );
        let check_leader_scheduler = check_leader_worker.start(CHECK_LEADER_THREAD_PREFIX, check_leader_runner);
        ...
        if self.core.config.resolved_ts.enable {
            let mut rts_worker = Box::new(LazyWorker::new(RESOLVED_TS_WORKER_THREAD_PREFIX));
...
impl<CER: ConfiguredRaftEngine> TikvServer<CER> {
    fn init_engines(
        ...
        let cdc_worker = Box::new(LazyWorker::new(CDC_THREAD_PREFIX));
