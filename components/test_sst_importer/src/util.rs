...
pub fn must_acquire_sst_lease(client: &ImportSstClient, meta: &SstMeta, ttl: Duration) {
    let resp = send_acquire_sst_lease(client, meta, ttl).unwrap();
...
pub fn must_release_sst_lease(client: &ImportSstClient, meta: &SstMeta) {
    let resp = send_release_sst_lease(client, meta).unwrap();
...
fn send_release_sst_lease(client: &ImportSstClient, meta: &SstMeta) -> Result<LeaseResponse> {
    let region_id = meta.get_region_id();
