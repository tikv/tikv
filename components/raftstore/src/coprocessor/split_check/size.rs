    ...
    E: KvEngine,
{
    fn on_kv(&mut self, _: &mut ObserverContext<'_>, entry: &KeyEntry) -> bool {
        ...
        if self.current_size > self.split_size && !over_limit {
            ...
            // the split key would be pushed this time, but the entry size for this time should not
            // be ignored.
...
pub mod tests {
    ...
    fn test_generate_bucket_impl(cfs_with_range_prop: &[CfName], data_cf: CfName, mvcc: bool) {
        ...
        for i in 0..2000 {
            // if not mvcc, kv size is (6+1)*2 = 14, given bucket size is 3000, expect each bucket
            // has about 210 keys if mvcc, kv size is about 18*2 = 36, expect each
        ...
        for i in 0..2000 {
            // kv size is (6+1)*2 = 14, given bucket size is 3000, expect each bucket has about 210
            // keys if mvcc, kv size is about 18*2 = 36, expect each bucket has about 80
            // keys
