...
use file_system::{
    ...
    file_exists, get_file_size, sync_dir, File, IoBytes, IoBytesTracker, IoType, Metadata,
    OpenOptions, WithIoType,
};
...
impl SnapshotFiles {
    ...
    pub fn build_plain_cf_file<EK: KvEngine>(
        ...
        cf_file.kv_count = stats.key_count;
...
impl Snapshot {
    ...
    pub fn build<EK: KvEngine>(
        ...
        for (cf_enum, cf) in SNAPSHOT_CFS_ENUM_PAIR {
            ...
            SNAPSHOT_CF_SIZE
                .get(*cf_enum)
                .observe(cf_stats.total_kv_size as f64);
            info!(
                "scan snapshot of one cf";
                "region_id" => region.get_id(),
                "snapshot" => &info.display_path,
                "cf" => cf,
                "key_count" => cf_stats.key_count,
                "size" => cf_stats.total_kv_size,
            );
        }
    ...
    pub fn apply<EK: KvEngine>(&self, options: ApplyOptions<EK>) -> Result<()> {
        ...
        for cf_file in &mut data.cf_files {
            ...
            if plain_file_used(cf_file.cf) {
                ...
                snap_io::apply_sst_cf_files_by_ingest(&clone_file_paths, &options.db, cf)?;
                // TODO(@Connor1996): Make it compatible
                // coprocessor_host.post_apply_sst_from_snapshot(&region,
                // cf, file.path);
            }
        }
        Ok(())
    ...
    pub fn total_count(&self) -> u64 {
        ...
        match *inner {
            SnapshotInner::Sending { ref data, .. } => {
                data.cf_files.iter().map(|cf| cf.kv_count as u64).sum()
            }
...
pub mod tests {
    ...
    // assert_eq!(cf_files.len(), super::SNAPSHOT_CFS.len() * 2); // each CF has two
    // snapshot files; for (i, cf_file_meta) in
    // meta.get_cf_files().iter().enumerate() { let cf_file_idx = i / 2;
    // let size_idx = i % 2;
    ...
    // max_file_size); let mut s1 = Snapshot::new_for_building(dir.path(), &key,
    // &mgr_core).unwrap(); assert!(!s1.exists());
    //
    // let _ = s1
    // .build(&db, &snapshot, &region, true, false, UnixSecs::now())
    // .unwrap();
    // assert!(s1.exists());
    //
    // let mut s2 = Snapshot::new_for_building(dir.path(), &key,
    // &mgr_core).unwrap(); assert!(s2.exists());
    //
    // let _ = s2
    // .build(&db, &snapshot, &region, true, false, UnixSecs::now())
    // .unwrap();
    ...
    // checksum.
    // fn corrupt_snapshot_checksum_in<T: Into<PathBuf>>(dir: T) ->
    // Vec<SnapshotMeta> { let dir_path = dir.into();
    // let mut res = Vec::new();
    ...
    // assert!(from.exists());
    //
    // let mut to = Snapshot::new_for_receiving(to_dir.path(), key, mgr,
    // snapshot_meta).unwrap();
    //
    // assert!(!to.exists());
    ...
    // let mut s1 = Snapshot::new_for_building(dir.path(), &key,
    // &mgr_core).unwrap(); assert!(!s1.exists());
    //
    // let snap_data = s1
    // .build(&db, &snapshot, &region, true, false, UnixSecs::now())
    ...
    //
    // let mut s2 = Snapshot::new_for_applying(dst_dir.path(), &key,
    // &mgr_core).unwrap(); assert!(s2.exists());
    //
    // let dst_db_dir = Builder::new()
    ...
    // .tempdir()
    // .unwrap();
    // let dst_db: KvTestEngine = open_test_empty_db(dst_db_dir.path(), None,
    // None).unwrap(); let options = ApplyOptions {
    // db: dst_db,
    // region,
    // abort: Arc::new(AtomicUsize::new(JOB_STATUS_RUNNING)),
    // write_batch_size: TEST_WRITE_BATCH_SIZE,
    ...
    // .tempdir()
    // .unwrap();
    // let db: KvTestEngine = open_test_db_with_100keys(db_dir.path(), None,
    // None).unwrap(); let snapshot = db.snapshot();
    //
    // let dir = Builder::new()
    ...
    // Snapshot::new_for_receiving(dst_dir.path(), &key, &mgr_core,
    // snap_data.take_meta()) .unwrap_err();
    // }
    ...
    // let mut s2 =
    // Snapshot::new_for_receiving(&path, &key1, &mgr_core,
    // snap_data.get_meta().clone()) .unwrap();
    ...
    // let s4 =
    // Snapshot::new_for_receiving(&path, &key2, &mgr_core,
    // snap_data.take_meta()).unwrap();
    //
    // assert!(s1.exists());
    // assert!(s2.exists());
    // assert!(!s3.exists());
    ...
    // .collect();
    // let engine =
    // get_test_db_for_regions(&kv_path, None, None, Some(kv_cf_opts),
    // &regions).unwrap();
    //
    ...
    // let region = gen_test_region(1, 1, 1);
    //
    // Test one snapshot can be built multi times. DataKeyManager should be
    // handled correctly.
    // for _ in 0..2 {
    ...
    // let builder =
    // SnapManagerBuilder::default().enable_receive_tablet_snapshot(true);
    // let snap_dir = Builder::new()
    ...
    // fn test_from_path() {
    // let snap_dir =
    // Builder::new().prefix("test_from_path").tempdir().unwrap();
    ...
    // limiter.finish_recv(10); // calling finish_recv() on an empty limiter is
    // fine. assert!(limiter.try_recv(1)); // first recv should succeed
    //
