    ...
    EK: KvEngine,
{
    ...
    fn handle_prewrite<W: WriteBatch<EK>>(
        &mut self,
        wb: &mut W,
        req: &Request,
    ) -> Result<Response> {
    ...
    fn handle_commit<W: WriteBatch<EK>>(
        &mut self,
        engine: &EK,
        wb: &mut W,
        req: &Request,
        ...
        let lock = txn_types::Lock::parse(&lock_value).unwrap_or_else(|e| {
            panic!(
                ...
                self.tag,
                log_wrappers::Value::key(&key),
                log_wrappers::Value::value(&lock_value),
                e,
            )
        });
        ...
        wb.put_cf(CF_WRITE, &write_key, write_value.as_slice())
            .unwrap_or_else(|e| {
                panic!(
                    "{} failed to write ({}, {}) to cf {}: {:?}",
                    self.tag,
                    log_wrappers::Value::key(&write_key),
                    log_wrappers::Value::value(&write_value),
                    CF_WRITE,
                    e
                )
            });
