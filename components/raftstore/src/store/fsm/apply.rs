            ...
            match res {
                ...
                ApplyResult::Yield | ApplyResult::WaitMergeSource(_) => {
                    ...
                    if let ApplyResult::WaitMergeSource(logs_up_to_date) = res {
                        ...
                        fail_point!("on_wait_merge_source_entered");
    ...
    EK: KvEngine,
{
    ...
    fn on_wait_merge_source_timeout(&mut self, ctx: &mut ApplyContext<EK>) {
        ...
        let drain_msg = |msg: Box<Msg<EK>>, pending_regs: &mut Vec<_>| {
            match *msg {
                Msg::Registration(reg) => pending_regs.push(reg),
                Msg::Apply { mut apply, .. } => {
        ...
        for msg in yield_msgs {
            drain_msg(msg, &mut pending_registrations);
        }
        while let Ok(msg) = self.receiver.try_recv() {
            drain_msg(msg, &mut pending_registrations);
        }
        // Re-route any Registration messages so the region is not left without
        // an apply delegate after this peer is recreated via snapshot.
        for reg in pending_registrations {
            ctx.router
                .schedule_task(reg.region.get_id(), Msg::Registration(reg));
        }
    }
