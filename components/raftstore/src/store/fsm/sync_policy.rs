...
impl SyncAction {
    pub fn new(raft_engine: Engines, router: RaftRouter<RocksEngine>) -> SyncAction {
        SyncAction {
            raft_engine,
            router,
        }
...
impl Action for SyncAction {
    ...
    fn sync_raft_engine(&self) {
        self.raft_engine.sync_raft().unwrap_or_else(|e| {
...
pub fn new_sync_policy(
    raft_engine: Engines,
    router: RaftRouter<RocksEngine>,
    delay_sync_enabled: bool,
    delay_sync_us: i64,
) -> SyncPolicy<SyncAction> {
    SyncPolicy::new(
