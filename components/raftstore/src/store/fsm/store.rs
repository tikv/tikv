...
use crate::store::{
    ...
    ReadProgress, SignificantMsg, SnapManager, StoreMsg, StoreTick,
};
use crate::Result;
...
pub struct StoreMeta {
    ...
    /// region_id -> RegionMeta
    pub region_metas: HashMap<u64, RegionMeta>,
}
...
impl<EK: KvEngine, ER: RaftEngine, T> RaftPollerBuilder<EK, ER, T> {
    ...
    fn init(&mut self) -> Result<Vec<SenderFsmPair<EK, ER>>> {
        ...
        kv_engine.scan_cf(CF_RAFT, start_key, end_key, false, |key, value| {
            ...
            meta.region_metas
                .insert(region.get_id(), peer.peer.region_meta.clone());
                ...
                if let Some((term, leader)) = meta.leaders.get(&leader_info.region_id) {
                    if let Some(region) = meta.regions.get(&leader_info.region_id) {
                        if *term == leader_info.term
                            ...
                            .is_ok()
                        {
                            if let Some(pr) = meta.region_metas.get(&leader_info.region_id) {
                                pr.forward_safe_ts(leader_info.get_applied_index(), leader_info.get_safe_ts());
                            }
...
impl RegionMeta {
    ...
    pub fn forward_safe_ts(&self, apply_index: u64, ts: u64) {
        ...
        if self.applied_index.load(Ordering::Relaxed) < apply_index {
            ...
            pending_ts.push_back((apply_index, ts));
        } else {
            self.safe_ts.fetch_max(ts, Ordering::Relaxed);
        }
    }
