    ...
    fn build(
        ...
        if let Some(req) = self.request.take() {
            ...
            if self.callbacks.len() == 1 {
                ...
                let pw_cb = self.pw_callbacks.pop();
                return Some((req, cb, pw_cb));
            }
...
impl<'a, T: Transport, C: PdClient> PeerFsmDelegate<'a, T, C> {
            ...
            match m {
                ...
                PeerMsg::RaftCommand(cmd) => {
                    ...
                    if self.fsm.batch_req_builder.can_batch(&cmd.request, req_size) {
                        self.fsm.batch_req_builder.add(
                            cmd.request,
                            req_size,
                            cmd.callback,
                            cmd.pw_callback,
                        ...
                        self.propose_batch_raft_command();
                        self.propose_raft_command(cmd.request, cmd.callback, cmd.pw_callback)
                    }
                }
    ...
    fn propose_batch_raft_command(&mut self) {
        if let Some((req, cb, pw_cb)) = self
            .fsm
            .batch_req_builder
            .build(&mut self.ctx.raft_metrics.propose)
        {
            self.propose_raft_command(req, cb, pw_cb)
        }
    }
    ...
    fn propose_raft_command(
        ...
        if self.fsm.peer.propose(self.ctx, cb, msg, resp, pw_cb) {
            self.fsm.has_ready = true;
        }
