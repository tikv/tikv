...
impl ReadDelegate {
    ...
    fn maybe_renew_lease_advance<EK: KvEngine>(&self, router: &dyn CasualRouter<EK>, ts: Timespec) {
        if !self
            .leader_lease
            .as_ref()
...
mod tests {
    ...
    fn test_read() {
        ...
        assert_eq!(
            TLS_LOCAL_READ_METRICS.with(|m| m
                .borrow()
                .reject_reason
                .store_id_mismatch
                .get()),
        ...
        assert_eq!(TLS_LOCAL_READ_METRICS.with(
            |m| m.borrow().reject_reason.term_mismatch.get()
