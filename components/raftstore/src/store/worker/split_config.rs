...
// abs(sample.left-sample.right)/(sample.right+sample.left). It will be used
...
// satisfied, we will try to split the region according to its CPU load, then
// these parameters below will start to work. When the gRPC poll thread CPU
// usage (over the past `detect_times` seconds by default) is higher than gRPC
// poll thread count * `DEFAULT_GRPC_THREAD_CPU_OVERLOAD_THRESHOLD_RATIO`,
...
// thread count *
// `DEFAULT_UNIFIED_READ_POOL_THREAD_CPU_OVERLOAD_THRESHOLD_RATIO`, the
// CPU-based split will try to check and record the top hot CPU region.
