...
// We get balance score by abs(sample.left-sample.right)/(sample.right+sample.left). It will be used
// to measure left and right balance
const DEFAULT_SPLIT_BALANCE_SCORE: f64 = 0.25;
// We get contained score by sample.contained/(sample.right+sample.left+sample.contained). It will
// be used to avoid to split regions requested by range.
const DEFAULT_SPLIT_CONTAINED_SCORE: f64 = 0.5;

// If the `split_balance_score` and `split_contained_score` above could not be satisfied, we will
// try to split the region according to its CPU load, then these parameters below will start to
// work. When the gRPC poll thread CPU usage (over the past `detect_times` seconds by default) is
// higher than gRPC poll thread count * `DEFAULT_GRPC_THREAD_CPU_OVERLOAD_THRESHOLD_RATIO`,
// the CPU-based split won't be triggered no matter if the
// `DEFAULT_UNIFIED_READ_POOL_THREAD_CPU_OVERLOAD_THRESHOLD_RATIO` and
// `REGION_CPU_OVERLOAD_THRESHOLD_RATIO` are exceeded to prevent from increasing the gRPC poll CPU
// usage.
const DEFAULT_GRPC_THREAD_CPU_OVERLOAD_THRESHOLD_RATIO: f64 = 0.5;
// When the Unified Read Poll thread CPU usage is higher than Unified Read Poll thread count *
// `DEFAULT_UNIFIED_READ_POOL_THREAD_CPU_OVERLOAD_THRESHOLD_RATIO`, the CPU-based split will try to
// check and record the top hot CPU region.
...
// `REGION_CPU_OVERLOAD_THRESHOLD_RATIO` as a percentage of the Unified Read Poll, it will be added
// into the hot region list and may be split later as the top hot CPU region.
