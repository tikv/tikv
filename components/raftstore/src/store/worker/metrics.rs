...
lazy_static! {
    ...
    pub static ref SNAP_WAIT_HISTOGRAM_VEC: HistogramVec = register_histogram_vec!(
        "tikv_raftstore_snapshot_wait_duration_seconds",
        "Bucketed histogram of raftstore snapshot wait",
        &["type"],
        exponential_buckets(0.00001, 2.0, 26).unwrap()
    )
    .unwrap();
    pub static ref SNAP_WAIT_HISTOGRAM: SnapHistogram =
        auto_flush_from!(SNAP_WAIT_HISTOGRAM_VEC, SnapHistogram);
