...
fn test_calculate_adaptive_wait_duration_high_and_low_qps_paths() {
    ...
    t.worker.consecutive_low_batch_ratio_count = 0;
    // Low QPS (1000) => qps_pressure=0.025 < 0.1 => very low QPS =>
    // target=current*0.7
...
fn test_calculate_adaptive_wait_duration_futile_wait_reduction() {
    ...
    // After this call, consecutive_low_batch_ratio_count becomes 2 => futile
    // triggered
    t.worker.batch.recorder.wait_duration_hint = Duration::from_nanos(20_000);
...
fn test_adaptive_config_rejects_zero_threshold() {
    ...
    // Layer 1: Config::validate() rejects adaptive_high_qps_threshold == 0 at
    // startup.
    let mut cfg = Config::default();
    ...
    cfg_track
        .update(
            |cfg: &mut Config| -> std::result::Result<(), Box<dyn std::error::Error>> {
                cfg.adaptive_high_qps_threshold = 0;
                Ok(())
            },
        )
...
fn test_adaptive_worker_zero_threshold_no_panic() {
    ...
    // => Branch 2 (qps_pressure > 0.5 && current < hint), grows toward hint. No
    // panic.
...
fn test_adaptive_no_oscillation_high_qps_low_batch_at_hint() {
    ...
    // qps_pressure = 50000/40000 = 1.0 > 0.5 => futile blocked (even though
    // consecutive_low >= 2) Branch 2: qps_pressure > 0.5 && current < hint? No
    // (current == hint) Branch 5: hold steady => target = current
