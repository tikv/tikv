    ...
    ER: RaftEngine,
{
    ...
    fn should_send<C: WriteRouterContext<EK, ER>>(
        ...
        let need_correct_writer_id = {
            ...
            // senders. There exists one corner case should be tackled ==> when resizing
            // the size of async-ios, we will pause pollers of BatchRaftSystem, but there
            // maybe left several events(i.e. IoTasks) for scheduling. In this case,
            // `write_id` might be out of range. So, we should correct it by mod
            // `capacity` to avoid out of bound.
            // For examples, supposing that previous `senders.capacity()` == 4,
            // and `writer_id` is set with 3. If `writer_id` was not updated for
            // accessing the current senders when resizing the count of writers
