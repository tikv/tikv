...
use tikv_util::{error, info, safe_panic, time::Instant};
    ...
    ER: RaftEngine,
{
    ...
    fn should_send<C: WriteRouterContext<EK, ER>>(
        ...
        let need_correct_writer_id = {
            // Because the given `writer_id` is not atomically updated during the
            // progress of resizing, that is, the given `writer_id` is generated by
            // `% senders.capacity()` and not consistent with current capacity of
            // senders. It should be corrected when resizing the size of senders.
            self.writer_id >= async_io_pool_size
        };
...
pub struct WriteSenders<EK: KvEngine, ER: RaftEngine> {
    senders: Arc<Mutex<SenderVec<EK, ER>>>,
...
impl<EK: KvEngine, ER: RaftEngine> Clone for WriteSenders<EK, ER> {
    fn clone(&self) -> Self {
        // `Clone` is overried for updating write senders in `PollContext`, when
        // initializing it.
        let cached_senders = self.senders.lock().clone();
        Self {
...
impl<EK: KvEngine, ER: RaftEngine> WriteSenders<EK, ER> {
    pub fn new(senders: Arc<Mutex<SenderVec<EK, ER>>>) -> Self {
        let cached_senders = senders.lock().clone();
        WriteSenders {
            senders,
            cached_senders,
...
impl<EK: KvEngine, ER: RaftEngine> WriteSenders<EK, ER> {
    #[inline]
    pub fn update(&mut self, size: i64) {
        // -1 => all
        debug_assert!(size != 0);
        let senders = self.senders.lock().clone();
        if size > 0 {
            self.cached_senders = senders[0..size as usize].to_vec();
        } else {
            debug_assert_eq!(size, -1);
            self.cached_senders = senders;
        }
    }
...
mod tests {
    ...
    impl TestWriteRouter {
        fn new(config: Config) -> Self {
            ...
            Self {
                receivers,
                senders: WriteSenders::new(Arc::new(Mutex::new(senders))),
                config,
