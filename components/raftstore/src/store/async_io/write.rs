    ...
    T: Transport,
{
    ...
    fn update_adaptive_parameters(&mut self) {
        ...
        let now = Instant::now();
        // Update parameters every 100ms to avoid too frequent adjustments
        ...
        // Calculate average latency and QPS
        let avg_latency = self.latency_history.iter().sum::<Duration>() / self.latency_history.len() as u32;
        let avg_qps = self.qps_history.iter().sum::<u64>() / self.qps_history.len() as u64;
    ...
    fn calculate_adaptive_batch_size(&self, avg_latency: Duration, avg_qps: u64, current_size: usize) -> usize {
        ...
        let latency_baseline = if !self.latency_baseline_history.is_empty() {
            // Use the 90th percentile of historical data as baseline
        ...
        let qps_baseline = if !self.qps_baseline_history.is_empty() {
            // Use the 50th percentile of historical data as baseline
        ...
        let latency_ratio = if !latency_baseline.is_zero() {
        ...
        let qps_ratio = if qps_baseline > 0 {
        ...
        // 1. If latency is low (< 1.0) and QPS is high (> 1.0), increase batch size
        // 2. If latency is high (> 1.0), reduce batch size
        // 3. If QPS is low (< 1.0) and latency is high, reduce batch size
        // 4. If QPS is low (< 1.0) and latency is low, slightly reduce batch size to maintain low latency
        ...
        if latency_ratio > 2.0 {
            ...
            // Latency is much lower than baseline and QPS is high, increase batch size
            adjustment_factor = 1.5;
        } else if latency_ratio < 0.8 && qps_ratio > 1.2 {
            // Latency is lower than baseline and QPS is high, moderately increase batch size
            adjustment_factor = 1.3;
        } else if latency_ratio < 1.0 && qps_ratio > 2.0 {
            // Latency is lower than baseline and QPS is very high, moderately increase batch size
            adjustment_factor = 1.2;
        } else if qps_ratio < 0.5 && latency_ratio > 1.2 {
        ...
        // Ensure new size is within reasonable range (32KB to 16MB)
        new_size.clamp(32 * 1024, 16 * 1024 * 1024)
    }
    ...
    fn calculate_adaptive_wait_duration(&self, avg_latency: Duration, avg_qps: u64, current_duration: Duration) -> Duration {
        ...
        if latency_ratio > 2.0 {
            ...
            // Latency is lower than baseline and QPS is very high, moderately increase wait time
            adjustment_factor = 1.3;
        } else if qps_ratio < 0.5 && latency_ratio > 1.2 {
        ...
        // Ensure new wait time is within reasonable range (0 to 1ms)
        Duration::from_nanos(new_duration_nanos.clamp(0, 1_000_000))
    }
