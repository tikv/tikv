...
fn fmt_req(req: &Request) -> String {
    ...
    match cmd_type {
        CmdType::Get => format!(
            "{:?}{}",
            cmd_type,
            fmt_cf_key(req.get_get().get_cf(), req.get_get().get_key())
        ),
        CmdType::Put => format!(
            "{:?}{}",
            cmd_type,
            fmt_cf_key(req.get_put().get_cf(), req.get_put().get_key())
        ),
        CmdType::Delete => format!(
            "{:?}{}",
            cmd_type,
            fmt_cf_key(req.get_delete().get_cf(), req.get_delete().get_key())
        ),
        CmdType::Prewrite => format!(
            "{:?}({},{})",
            cmd_type,
            log_wrappers::hex_encode(req.get_prewrite().get_key()),
            log_wrappers::hex_encode(req.get_prewrite().get_lock())
        ),
        CmdType::ReadIndex => format!(
            "{:?}({},[{}])",
            cmd_type,
            req.get_read_index().get_start_ts(),
            req.get_read_index()
                .get_key_ranges()
                .iter()
                .map(|r| format!(
                    "{}:{}",
                    log_wrappers::hex_encode(r.get_start_key()),
                    log_wrappers::hex_encode(r.get_end_key())
                ))
                .collect::<Vec<String>>()
                .join(",")
        ),
