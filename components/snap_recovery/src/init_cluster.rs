...
pub fn enter_snap_recovery_mode(config: &mut TikvConfig) {
    ...
    // ebs volume has very poor performance during restore, it easy to cause the raft client timeout, at the same time clean up all message included significant message.
    // restore is not memory sensetive, we may keep messages as much as possible during the network disturbing in recovery mode
