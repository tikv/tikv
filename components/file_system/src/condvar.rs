...
/// Un-prioritized condition variable. Supports both synchronously or
...
/// That said, the total amount of waiting tasks is unchanged, meaning we won't
/// cause more damage than it should to system-wise performance.
/// Alternative: Sharing one single condvar at the cost of thundering-herd or
/// out-of-order wakeups.
#[derive(Debug)]
pub struct Condvar {
...
impl Condvar {
    ...
    fn dequeue_and_maybe_notify_next(&self, raw_node: &mut CondvarNode) {
        ...
        if let Some(prev) = prev {
            ...
            assert!(self.waiting.head.get().contains(&boxed_node));
            self.waiting.head.set(next);
        }
        if let Some(next) = next {
            unsafe {
            ...
            assert!(self.waiting.tail.get().contains(&boxed_node));
            self.waiting.tail.set(prev);
        }
    }
    ...
    pub async fn async_wait_timeout<'a, 'b, T>(
        ...
        self.enqueue(&mut node);
        // no risk of losing signal because `Notify` preserves state.
        std::mem::drop(guard);
        let timed_out = {
            let f = match node {
        ...
        for i in 0..wait_group_size * 3 {
            ...
            let t = std::thread::spawn(move || {
                ...
                if i % 3 == 0 {
                    ...
                    let (_guard, timed_out) = rt.block_on(condv.async_wait_timeout(
                        &mu,
                        guard,
