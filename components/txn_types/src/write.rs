...
impl WriteRef<'_> {
    ...
    /// - The `read_ts` is safe, which means it is not earlier than the current
    ///   GC safepoint.
    ///
    /// Return:
    /// Whether the `Write` record is valid, i.e. there is no GC fence or the GC
    /// fence does not point to any other version.
