...
impl<E: Engine, R: RegionInfoProvider + Clone + 'static> Endpoint<E, R> {
    pub fn new(
        ...
        let pool = ResizableRuntime::new("backup-worker", |new_size|{BACKUP_THREAD_POOL_SIZE_GAUGE.set(new_size as i64)}, utils::BackupRuntimeCreator::create_tokio_runtime);
        let rt = utils::BackupRuntimeCreator::create_tokio_runtime(config.io_thread_size, "backup-io").unwrap();
...
pub mod tests {
    ...
    fn test_control_thread_pool_adjust_keep_tasks() {
        ...
        let mut pool =  ResizableRuntime::new("bkwkr",|new_size:usize|{BACKUP_THREAD_POOL_SIZE_GAUGE.set(new_size as i64)}, utils::BackupRuntimeCreator::create_tokio_runtime);
        pool.adjust_with(3);
