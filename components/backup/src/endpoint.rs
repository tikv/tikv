...
use tikv_util::time::{Limiter, SlowTimer};
    ...
    fn check_active(&mut self, idle_threshold: Duration) {
        if self.last_active.elapsed() >= idle_threshold {
            ...
            if let Some(w) = self.workers.take() {
                ...
                drop(w);
                slow_log!(timer, "backup thread pool shutdown too long");
            }
        }
    }
}
            ...
            match res {
                Ok((mut files, stat)) => {
                    ...
                    for file in files.iter_mut() {
                        ...
                        file.set_start_version(request.start_ts);
                        file.set_end_version(request.end_ts);
                    }
