...
impl RfEngine {
    pub(crate) fn load_epoch(&mut self, ep: &mut Epoch, prev_has_state_file: bool) -> Result<u64> {
        ...
        if ep.has_wal_file {
            ...
            // Remove rlogs generated by last unfinished compact job. The truncated state in
            // background worker is emtpy at the beginning, so new compact job may not overwrite
            // these rlogs, we have to remove it manually.
            for (region_id, (first, end)) in ep.raft_log_files.lock().unwrap().drain() {
                let filename = raft_log_file_name(&self.dir, ep.id, region_id, first, end);
                fs::remove_file(filename)?;
            }
            if ep.has_state_file {
                let filename = states_file_name(&self.dir, ep.id);
                fs::remove_file(filename)?;
                ep.has_state_file = false;
            }
        } else {
