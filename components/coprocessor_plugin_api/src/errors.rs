...
///
/// If a plugin wants to return a custom error, e.g. an error in the business logic, the plugin
/// should return an appropriately encoded error in [`RawResponse`]; in other words, plugins are
/// responsible for their error handling by themselves.
#[derive(Debug)]
pub enum PluginError {
    ...
    /// Errors that can not be handled by a coprocessor plugin but should instead be returned to
    /// the client.
    ///
