    ...
    pub fn get_logical_scalar_ref(&self, logical_index: usize) -> ScalarValueRef<'_> {
        match self {
            RpnStackNode::Vector { value, .. } => match value {
                RpnStackNodeVectorValue::Generated { value } => value.get_scalar_ref(logical_index),
                RpnStackNodeVectorValue::Ref {
                    physical_value,
                    logical_rows,
                } => physical_value.get_scalar_ref(logical_rows[logical_index]),
            },
        ...
        for node in self.as_ref() {
            match node {
                ...
                RpnExpressionNode::FnCall {
                    ...
                    stack.push(RpnStackNode::Vector {
                        value: RpnStackNodeVectorValue::Generated { value: ret },
                        field_type: ret_field_type,
                    });
                }
            }
