...
use kvproto::brpb::{BackupMeta, File as BackupFile};
...
impl<'a> SstParquetExporter<'a> {
    ...
    pub fn export_backup_meta(
        ...
        self.export_from_meta(meta, output_prefix)
    }
    ...
    fn export_from_meta(&mut self, meta: BackupMeta, output_prefix: &str) -> Result<ExportReport> {
...
fn datum_to_cell(column: &ColumnSchema, datum: &Datum) -> Result<CellValue> {
    match column.parquet_type {
        ColumnParquetType::Int64 => match datum {
            ...
            Datum::U64(v) => Ok(CellValue::Int64(*v as i64)),
...
mod tests {
    ...
    use kvproto::brpb::{BackupMeta, File as BackupFile, Schema};
    use parquet::{
