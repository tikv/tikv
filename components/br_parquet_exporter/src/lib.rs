...
/// Re-exported Parquet compression codec type.
...
/// Schema types used by the BR Parquet exporter.
pub use schema::{ColumnParquetType, ColumnSchema, TableSchema};
use writer::{CellValue, ParquetWriter};

/// Snapshot type used by [`exporter_metrics_snapshot`].
...
#[derive(Debug, Error)]
pub enum Error {
    /// Failed to read or write local/remote data.
    #[error("I/O error {0}")]
    Io(#[from] std::io::Error),
    /// Failed to write a Parquet file.
    #[error("Parquet error {0}")]
    Parquet(#[from] parquet::errors::ParquetError),
    /// Failed to read an SST file.
    #[error("SST error {0}")]
    Sst(#[from] engine_traits::Error),
    /// Failed to decode a protobuf message.
    #[error("Protobuf error {0}")]
    Protobuf(#[from] protobuf::error::ProtobufError),
    /// Failed to decode a TiDB datum.
    #[error("Codec error {0}")]
    Codec(#[from] tidb_query_datatype::codec::Error),
    /// Schema metadata was missing or invalid.
    #[error("Schema error {0}")]
    Schema(String),
    /// Backup input data was missing or invalid.
    #[error("Invalid backup: {0}")]
    Invalid(String),
    /// Any other error from dependencies.
    #[error(transparent)]
...
pub struct ExportOptions {
    ...
    /// Maximum number of SST files converted concurrently.
    pub sst_concurrency: usize,
    /// Whether to persist a checkpoint directory to skip already-exported
    /// files on re-runs.
    pub use_checkpoint: bool,
    /// Whether to emit Iceberg table metadata alongside Parquet files.
    pub write_iceberg_table: bool,
    /// Whether to write Parquet bloom filters for eligible columns.
...
pub struct ExportReport {
    /// Parquet files generated by the export.
    pub files: Vec<ParquetFileInfo>,
    /// Total number of rows exported across all files.
    pub total_rows: u64,
    /// Start version bound attached to exported files.
    pub start_version: u64,
    /// End version bound attached to exported files.
    pub end_version: u64,
}
...
pub struct IcebergFileMetrics {
    /// Total number of values per Iceberg field id.
    pub value_counts: HashMap<i32, u64>,
    /// Total number of null values per Iceberg field id.
    pub null_value_counts: HashMap<i32, u64>,
    /// Column lower bounds, encoded as Iceberg binary values keyed by field id.
    pub lower_bounds: HashMap<i32, Vec<u8>>,
    /// Column upper bounds, encoded as Iceberg binary values keyed by field id.
...
pub struct ParquetFileInfo {
    /// Object path or name in the destination storage.
    pub object_name: String,
    /// TiDB table id encoded in the exported file.
    pub table_id: i64,
    /// TiDB database name.
    pub db_name: String,
    /// TiDB table name.
    pub table_name: String,
    /// Number of rows written to this Parquet file.
    pub row_count: u64,
    /// Total size in bytes uploaded to storage.
    pub size: u64,
    /// SHA-256 checksum of the uploaded Parquet payload.
    pub sha256: Vec<u8>,
    /// Inclusive start key encoded in BR metadata.
    pub start_key: Vec<u8>,
    /// Exclusive end key encoded in BR metadata.
    pub end_key: Vec<u8>,
    /// Optional metrics used when generating Iceberg manifests.
