...
/// A wrapper around configurable values that tracks whether they were
/// deserialized from configuration.
///
/// This struct extends both basic Rust types (like numbers, booleans) and
/// TiKV's internal types (like `ReadableSize` and `ReadableDuration`) with the
/// ability to track whether their values came from configuration
/// deserialization or were set programmatically. This is particularly useful
/// for:
///
/// * Configuration validation - determining if a value was explicitly set by
///   the user
/// * Dynamic configuration updates - identifying which values need to be
///   updated
/// * Default value handling - distinguishing between default and configured
///   values
///
/// # Type Parameters
///
/// * `T` - The underlying type being wrapped. Must implement:
///   * `Default` - For creating default values
///   * `Copy` and `Clone` - For efficient value handling
///   * `PartialEq` and `PartialOrd` - For value comparison
///
///   This includes both basic Rust types and TiKV's internal types like:
///   * `ReadableSize` - For size configurations (e.g., memory limits)
///   * `ReadableDuration` - For time-based configurations (e.g., timeouts)
///
/// # Fields
///
/// * `0` - The actual value of type `T`
/// * `1` - A boolean flag indicating whether this value was deserialized from
///   configuration
///
/// # Examples
///
/// ```
/// use tikv_util::config::{ConfigValueExt, ReadableDuration, ReadableSize};
///
/// // Basic Rust types
/// let default = ConfigValueExt::<u64>::default();
/// assert!(!default.is_deserialized());
/// assert_eq!(default.val(), 0);
///
/// let manual = ConfigValueExt::from(42_u64);
/// assert!(!manual.is_deserialized());
/// assert_eq!(manual.val(), 42);
///
/// // TiKV types
/// let size = ConfigValueExt::from(ReadableSize::kb(1024));
/// assert!(!size.is_deserialized());
/// assert_eq!(size.val(), ReadableSize::kb(1024));
///
/// let duration = ConfigValueExt::from(ReadableDuration::hours(1));
/// assert!(!duration.is_deserialized());
/// assert_eq!(duration.val(), ReadableDuration::minutes(60));
///
/// // Values from configuration
/// let config: ConfigValueExt<u64> = serde_json::from_str("100").unwrap();
/// assert!(config.is_deserialized());
/// assert_eq!(config.val(), 100);
/// ```
///
/// # Serialization
///
/// When serialized, only the underlying value is included, making it compatible
/// with both basic types and TiKV's internal types. The deserialized flag is
/// not preserved in the serialized form, ensuring clean configuration files.
///
/// # Deserialization
///
/// When deserialized from configuration, the value is set to the deserialized
/// input and the deserialized flag is set to `true`. This allows distinguishing
/// between values that came from configuration and those set programmatically.
#[derive(Debug, Clone, Copy, PartialEq, PartialOrd)]
pub struct ConfigValueExt<T>(pub T, bool)
where
    T: Default + Copy + Clone + PartialEq + PartialOrd;

impl<T> Default for ConfigValueExt<T>
where
    T: Default + Copy + Clone + PartialEq + PartialOrd,
{
    /// Creates a default `ConfigValueExt` with the default value of `T`
    /// and `false` for the deserialized flag.
    ///
    /// This is used when a value hasn't been explicitly set or deserialized
    /// from configuration. Works with both basic types and TiKV's internal
    /// types.
    fn default() -> Self {
...
impl<T> Serialize for ConfigValueExt<T>
where
    T: Default + Copy + Clone + PartialEq + PartialOrd + Serialize,
{
    /// Serializes only the underlying value, ignoring the deserialized flag.
    ///
    /// This ensures that the serialized format remains compatible with both
    /// basic types and TiKV's internal types. The deserialized status is
    /// not preserved in the serialized form, keeping configuration files
    /// clean and simple.
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
...
impl<'de, T> Deserialize<'de> for ConfigValueExt<T>
where
    T: Default + Copy + Clone + PartialEq + PartialOrd + Deserialize<'de>,
{
    /// Deserializes a `ConfigValueExt` from a value that can be
    /// deserialized into type `T`.
    ///
    /// The deserialized flag is set to `true` to indicate that this value came
    /// from configuration rather than being a default value or set
    /// programmatically. This is useful for validating configuration
    /// changes and determining whether a value needs to be updated.
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        let size = T::deserialize(deserializer)?;
        Ok(ConfigValueExt::<T>(size, true))
    }
}

impl<T> From<T> for ConfigValueExt<T>
where
    T: Default + Copy + Clone + PartialEq + PartialOrd,
{
    /// Creates a `ConfigValueExt` from a value of type `T` with the
    /// deserialized flag set to `false`.
    ///
    /// This is used when converting a value to `ConfigValueExt`
    /// programmatically, rather than through deserialization. Works with
    /// both basic types and TiKV's internal types.
    fn from(val: T) -> Self {
        ConfigValueExt::<T>(val, false)
    }
}

impl<T> ConfigValueExt<T>
where
    T: Default + Copy + Clone + PartialEq + PartialOrd,
{
    /// Returns the underlying value.
    ///
    /// This provides access to the wrapped value while maintaining the
    /// deserialized status. Works with both basic types and TiKV's internal
    /// types like `ReadableSize` and `ReadableDuration`.
    pub fn val(&self) -> T {
        self.0
    }

    /// Returns whether this value was deserialized from configuration.
    ///
    /// This can be used to distinguish between:
    /// * Default values (not deserialized)
    /// * Programmatically set values (not deserialized)
    /// * Values explicitly set in configuration (deserialized)
    ///
    /// Particularly useful for validating configuration changes and determining
    /// whether a value needs to be updated.
...
mod tests {
    ...
    #[test]
    fn test_config_value_ext() {
        let t = ConfigValueExt::<u8>::default();
        assert_eq!(t, ConfigValueExt::<u8>(0_u8, false));
        let t = ConfigValueExt::from(8_u32);
        assert_eq!(t, ConfigValueExt::<u32>(8_u32, false));
        assert_eq!(t.0, 8_u32);
        assert_eq!(t.val(), 8_u32);
        assert!(!t.is_deserialized());
        let t = ConfigValueExt::from(-1_i32);
        assert_eq!(t, ConfigValueExt::<i32>(-1_i32, false));
        let t = ConfigValueExt::from(u64::MAX);
        assert_eq!(t, ConfigValueExt::<u64>(u64::MAX, false));
        let t = ConfigValueExt::from(1.234_f64);
        assert_eq!(t, ConfigValueExt::<f64>(1.234_f64, false));
        let t = ConfigValueExt::from(false);
        assert_eq!(t, ConfigValueExt::<bool>(false, false));
        // For types defined in TiKV
        let t = ConfigValueExt::<ReadableSize>::from(ReadableSize::kb(1024));
        assert_eq!(t.val(), ReadableSize::kb(1024));
        assert!(!t.is_deserialized());
        let t = ConfigValueExt::<ReadableDuration>::from(ReadableDuration::hours(1));
        assert_eq!(t.val(), ReadableDuration::minutes(60));
    }

    #[test]
    fn test_parse_config_value_ext() {
        // For basic types.
        {
            #[derive(Serialize, Deserialize)]
            struct Holder {
                val: ConfigValueExt<u64>,
            }

            let legal_cases = vec![(0, "0"), (256, "256")];
            for (v, exp) in legal_cases {
                let c = Holder {
                    val: ConfigValueExt::<u64>::from(v),
                };
                assert!(!c.val.is_deserialized());
                let res_str = toml::to_string(&c).unwrap();
                let exp_str = format!("val = {}\n", exp);
                assert_eq!(res_str, exp_str);
                let res_val: Holder = toml::from_str(&exp_str).unwrap();
                assert_eq!(res_val.val.val(), c.val.val());
                assert!(res_val.val.is_deserialized());
            }
        }
        // ReadableSize
        {
            ...
            for (size, exp) in legal_cases {
                let c = SizeHolder {
                    s: ConfigValueExt::from(ReadableSize(size)),
                };
                assert!(!c.s.is_deserialized());
                let res_str = toml::to_string(&c).unwrap();
                let exp_str = format!("s = {:?}\n", exp);
                assert_eq!(res_str, exp_str);
                let res_size: SizeHolder = toml::from_str(&exp_str).unwrap();
                assert_eq!(res_size.s.val().0, size);
                assert!(res_size.s.is_deserialized());
            }
        }
        // ReadableDuration
        {
            #[derive(Serialize, Deserialize)]
            struct DurHolder {
                d: ConfigValueExt<ReadableDuration>,
            }
            ...
            for (secs, us, exp) in legal_cases {
                let d = DurHolder {
                    d: ConfigValueExt::from(ReadableDuration(Duration::new(secs, us * 1_000))),
                };
                assert!(!d.d.is_deserialized());
                let res_str = toml::to_string(&d).unwrap();
                let exp_str = format!("d = {:?}\n", exp);
                assert_eq!(res_str, exp_str);
                let res_dur: DurHolder = toml::from_str(&exp_str).unwrap();
                assert_eq!(res_dur.d.val(), d.d.val());
                assert!(res_dur.d.is_deserialized());
            }
        }
    }
