...
impl QuotaLimiter {
    ...
    pub fn new(
        ...
        let foreground_cputime_limiter = Limiter::builder(Self::speed_limit(foreground_cpu_quota as f64 * 1000_f64))
            .refill(CPU_LIMITER_REFILL_DURATION)
            .build();

        let foreground_write_bandwidth_limiter = Limiter::new(Self::speed_limit(foreground_write_bandwidth.0 as f64));

        let foreground_read_bandwidth_limiter = Limiter::new(Self::speed_limit(foreground_read_bandwidth.0 as f64));

        let background_cputime_limiter = Limiter::builder(Self::speed_limit(background_cpu_quota as f64 * 1000_f64))
            .refill(CPU_LIMITER_REFILL_DURATION)
            .build();

        let background_write_bandwidth_limiter = Limiter::new(Self::speed_limit(background_write_bandwidth.0 as f64));

        let background_read_bandwidth_limiter = Limiter::new(Self::speed_limit(background_read_bandwidth.0 as f64));
...
impl ConfigManager for QuotaLimitConfigManager {
    fn dispatch(
        ...
        if let Some(cpu_limit) = change.get("foreground_cpu_time") {
            self.quota_limiter.set_foreground_cpu_time_limit(cpu_limit.into());
        }
        ...
        if let Some(cpu_limit) = change.get("background_cpu_time") {
            self.quota_limiter.set_background_cpu_time_limit(cpu_limit.into());
        }
        if let Some(write_bandwidth) = change.get("background_write_bandwidth") {
            self.quota_limiter.set_background_write_bandwidth_limit(write_bandwidth.clone().into())
        }
        if let Some(read_bandwidth) = change.get("background_read_bandwidth") {
            self.quota_limiter.set_background_write_bandwidth_limit(read_bandwidth.clone().into());
        }
