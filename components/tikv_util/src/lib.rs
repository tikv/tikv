...
pub fn set_panic_hook(panic_abort: bool, data_dir: &str) {
    ...
    panic::set_hook(Box::new(move |info: &panic::PanicInfo<'_>| {
        ...
        // terminal logger.
        // When the old global async logger is replaced, the old async guard will be taken and
        // dropped. In the drop() the async guard, it waits for the finish of the remaining
        // logs in the async logger.
