...
#[rpn_fn]
#[inline]
pub fn like<C: Collator, TargetCharset: Charset, PatternCharset: Charset>(
    target: BytesRef,
    pattern: BytesRef,
    escape: &i64,
) -> Result<Option<i64>> {
    ...
    while px < pattern.len() || tx < target.len() {
        if let Some((c, mut poff)) = PatternCharset::decode_one(&pattern[px..]) {
            let code: u32 = c.into();
            if code == '_' as u32 {
                if let Some((_, toff)) = TargetCharset::decode_one(&target[tx..]) {
                    px += poff;
                    tx += toff;
                    continue;
                }
                ...
                px += poff;
                next_tx = tx;
                next_tx += if let Some((_, toff)) = TargetCharset::decode_one(&target[tx..]) {
                    toff
                } else {
                    1
                };
                ...
                if code == escape && px + poff < pattern.len() {
                    px += poff;
                    poff = if let Some((_, off)) = PatternCharset::decode_one(&pattern[px..]) {
                        off
                    } else {
                        break;
                    }
...
mod tests {
    ...
    fn test_like_wide_character() {
        let cases = vec![
            ...
            // When the new collation framework is not enabled, the collation will always be binary
            // Some related tests are added here
            (
                r#"夏威夷吉他"#,
                r#"_____"#,
                '\\',
