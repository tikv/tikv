...
fn rand_with_seed_first_gen(seed: Option<&i64>) -> Result<Option<Real>> {
    let mut rng = super::utils::MySQLRng::new_with_seed(seed.cloned().unwrap_or(0));
    let res = rng.gen();
    Ok(Real::new(res).ok())
}
...
thread_local! {
   static MYSQL_RNG: RefCell<super::utils::MySQLRng> = RefCell::new(super::utils::MySQLRng::new())
}
