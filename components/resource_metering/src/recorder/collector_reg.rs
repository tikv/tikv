...
impl CollectorRegHandle {
    ...
    /// - When `as_observer` is false, the recorder will respect it and begin to profile if it's off
    ///   before. In other words, if there is at least one non-observed collector, the recorder will
    ///   keep running.
    /// - When `as_observer` is true, whether the recorder to be on or off won't depend on if the
    ///   collector exists.
