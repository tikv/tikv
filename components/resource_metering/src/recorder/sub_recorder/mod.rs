...
/// This trait defines a general framework that works at a certain frequency.
/// Typically, it describes the recorder(sampler) framework for a specific
/// resource.
///
/// [Recorder] will maintain a list of sub-recorders, driving all sub-recorders
/// to work according to the behavior described in this trait.
pub trait SubRecorder: Send {
    /// This function is called at a fixed frequency. (A typical frequency is
    /// 99hz.)
    ///
    /// The [RawRecords] and [LocalStorage] map of all threads will be passed in
    /// through parameters. We need to collect resources (may be from each
    /// `LocalStorage`) and write them into `RawRecords`.
    ///
    /// The implementation needs to sample the resource in this function (in
    /// general).
    ///
