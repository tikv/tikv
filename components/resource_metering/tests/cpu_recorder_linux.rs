...
use resource_metering::cpu::recorder::handle::RecorderHandle;
use resource_metering::cpu::recorder::records::CpuRecords;
use resource_metering::cpu::recorder::TEST_TAG_PREFIX;
use resource_metering::cpu::thread::{CpuTimeInstant, ThreadId};
use resource_metering::tag::{ResourceMeteringTag, TagInfos};
use tikv_util::defer;
        ...
        let handle = std::thread::spawn(|| {
            ...
            for op in ops {
                match op {
                    ...
                    Operation::CpuHeavy(ms) => {
                        let begin = CpuTimeInstant::now(&thread_id).unwrap();

                        loop {
                            Self::heavy_job();
                            let now = CpuTimeInstant::now(&thread_id).unwrap();
                            let delta_ms = now.minus_ms(&begin);
