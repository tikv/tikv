...
fn test_no_resolved_ts_before_downstream_initialized() {
    ...
    for i in 0..2 {
        ...
        block_on((&mut req_tx).sink_compat().send((req, WriteFlags::default()))).unwrap();
        req_txs.push(req_tx);
    ...
    let th = thread::spawn(move || {
        ...
        for _ in 0..10 {
            ...
            assert!(recv_timeout(&mut (&mut rx).compat(), Duration::from_secs(1)).is_err());
...
fn test_cdc_observed_before_incremental_scan_snapshot() {
    ...
    req_tx_0.send((req_0, WriteFlags::default())).wait().unwrap();
    ...
    let (mut req_tx, event_feed, receive_event) =
        new_event_feed(suite.get_region_cdc_client(region.id));
    let req = suite.new_changedata_request(region.id);
    req_tx.send((req, WriteFlags::default())).wait().unwrap();
    thread::sleep(Duration::from_secs(1));
    ...
    if let Ok(Some(Ok(event))) = recv_timeout(&mut (&mut rx).compat(), Duration::from_secs(1)) {
