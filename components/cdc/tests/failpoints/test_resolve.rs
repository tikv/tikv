...
fn test_check_leader_timeout() {
    ...
    let (mut req_tx, event_feed_wrap, receive_event) =
        new_event_feed(suite.get_region_cdc_client(region.get_id()));
    block_on(req_tx.send((req, WriteFlags::default()))).unwrap();
    // Pause checking leader in peer2 and peer3, then resolved ts must not be advanced
    let store2_fp = "before_check_leader_store_2";
    fail::cfg(store2_fp, "return").unwrap();
    let store3_fp = "before_check_leader_store_3";
    fail::cfg(store3_fp, "return").unwrap();
    // Receive resolved ts
    let mut last_resolved_ts = 0;
    let mut i = 0;
    loop {
    ...
    loop {
        ...
        if let Some(resolved_ts) = event.resolved_ts.as_ref() {
            let ts = resolved_ts.ts;
            // Check resolved ts is not advanced
            assert_eq!(ts, last_resolved_ts);
            i += 1;
        }
        if i > 10 {
            break;
    ...
    loop {
        ...
        if i > 15 {
            panic!(
                "resolved ts not be advanced, last_resolved_ts: {}",
                last_resolved_ts
            );
        }
    }
