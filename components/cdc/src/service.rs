...
use kvproto::{
    cdcpb::{
        ...
        ChangeDataRequest_oneof_request, FeaturesRequest, FeaturesResponse,
    },
    kvrpcpb::ApiVersion,
};
...
impl Service {
    ...
    fn start_connection_watchdog(
        ...
        let _ = pool.remote().spawn(async move {
...
impl ChangeData for Service {
    fn features(
        &mut self,
        ctx: RpcContext<'_>,
        _req: FeaturesRequest,
        sink: UnarySink<FeaturesResponse>,
    ) {
        let mut resp = FeaturesResponse::default();
        resp.set_features(
            vec![
                "event-feed-v2".to_owned(),
                EventFeedHeaders::STREAM_MULTIPLEXING.to_owned(),
            ]
            .into(),
        );
        ctx.spawn(
            sink.success(resp)
                .unwrap_or_else(|e| error!("failed to reply cdc features: {:?}", e)),
        );
    }
