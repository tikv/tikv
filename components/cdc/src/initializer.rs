...
impl<E: KvEngine> Initializer<E> {
    ...
    pub(crate) async fn async_incremental_scan<S: Snapshot + 'static>(
        ...
        let mut scanner = if kv_api == ChangeDataRequestKvApi::TiDb {
            ...
            let (raw_keyspace_start, raw_keyspace_end) = ApiV2::get_rawkv_range();
            iter_opt.set_lower_bound(raw_keyspace_start, DATA_KEY_PREFIX_LEN);
            iter_opt.set_upper_bound(raw_keyspace_end, DATA_KEY_PREFIX_LEN);
