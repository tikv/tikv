...
mod tests {
    ...
    use engine_rocks::{BlobRunMode, RocksEngine, RocksSnapshot};
    ...
    fn test_old_value_missing_range_cache_avoids_prev() {
        ...
        while let Some(entry) = scanner.next_entry().unwrap() {
            ...
            let old_value = match old_value {
                OldValue::SeekWrite(key) => resolve_old_value::<Arc<RocksSnapshot>>(
                    &key,
                    &mut old_value_cursors.write,
                    &mut old_value_cursors.default,
                    &mut old_value_stats,
                ),
                old_value => old_value,
            };
