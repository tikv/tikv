...
//!
//! - [`KvEngine`] - a key-value engine, and the primary type defined by this crate. Most code that
//!   uses generic engines will be bounded over a generic type implementing `KvEngine`. `KvEngine`
//!   itself is bounded by many other traits that provide collections of functionality, with the
//!   intent that as TiKV evolves it may be possible to use each trait individually, and to define
//!   classes of engine that do not implement all collections of features.
//!
//! - [`Snapshot`] - a view into the state of the database at a moment in time. For reading sets of
//!   consistent data.
//!
//! - [`Peekable`] - types that can read single values. This includes engines and snapshots.
//!
//! - [`Iterable`] - types that can iterate over the values of a range of keys, by creating
//!   instances of the TiKV-specific [`Iterator`] trait. This includes engines and snapshots.
//!
//! - [`SyncMutable`] and [`Mutable`] - types to which single key/value pairs can be written. This
//!   includes engines and write batches.
//!
//! - [`WriteBatch`] - types that can commit multiple key/value pairs in batches. A
//!   `WriteBatchExt::WriteBtach` commits all pairs in one atomic transaction. A
//!   `WriteBatchExt::WriteBatchVec` does not (FIXME: is this correct?).
//!
...
//! # Design notes
//!
//! - `KvEngine` is the main engine trait. It requires many other traits, which have many other
//!   associated types that implement yet more traits.
//!
//! - Features should be grouped into their own modules with their own traits. A common pattern is
//!   to have an associated type that implements a trait, and an "extension" trait that associates
//!   that type with `KvEngine`, which is part of `KvEngine's trait requirements.
//!
//! - For now, for simplicity, all extension traits are required by `KvEngine`. In the future it may
//!   be feasible to separate them for engines with different feature sets.
//!
//! - Associated types generally have the same name as the trait they are required to implement.
//!   Engine extensions generally have the same name suffixed with `Ext`. Concrete implementations
//!   usually have the same name prefixed with the database name, i.e. `Rocks`.
//!
//!   Example:
//!
//!   ```ignore
...
//!   ```
//!
//! - All engines use the same error type, defined in this crate. Thus engine-specific type
//!   information is boxed and hidden.
//!
//! - `KvEngine` is a factory type for some of its associated types, but not others. For now, use
//!   factory methods when RocksDB would require factory method (that is, when the DB is required to
//!   create the associated type - if the associated type can be created without context from the
//!   database, use a standard new method). If future engines require factory methods, the traits
//!   can be converted then.
//!
//! - Types that require a handle to the engine (or some other "parent" type) do so with either Rc
//!   or Arc. An example is EngineIterator. The reason for this is that associated types cannot
//!   contain lifetimes. That requires "generic associated types". See
//!
//!   - <https://github.com/rust-lang/rfcs/pull/1598>
//!   - <https://github.com/rust-lang/rust/issues/44265>
//!
//! - Traits can't have mutually-recursive associated types. That is, if `KvEngine` has a `Snapshot`
//!   associated type, `Snapshot` can't then have a `KvEngine` associated type - the compiler will
//!   not be able to resolve both `KvEngine`s to the same type. In these cases, e.g. `Snapshot`
//!   needs to be parameterized over its engine type and `impl Snapshot<RocksEngine> for
//!   RocksSnapshot`.
//!
//!
//! # The porting process
...
//! # Refactoring tips
//!
//! - Port modules with the fewest RocksDB dependencies at a time, modifying those modules's callers
//!   to convert to and from the engine traits as needed. Move in and out of the engine_traits world
//!   with the `RocksDB::from_ref` and `RocksDB::as_inner` methods.
//!
//! - Down follow the type system too far "down the rabbit hole". When you see that another
//!   subsystem is blocking you from refactoring the system you are trying to refactor, stop, stash
//!   your changes, and focus on the other system instead.
//!
//! - You will through away branches that lead to dead ends. Learn from the experience and try again
//!   from a different angle.
//!
//! - For now, use the same APIs as the RocksDB bindings, as methods on the various engine traits,
//!   and with this crate's error type.
//!
//! - When new types are needed from the RocksDB API, add a new module, define a new trait (possibly
//!   with the same name as the RocksDB type), then define a `TraitExt` trait to "mixin" to the
//!   `KvEngine` trait.
//!
//! - Port methods directly from the existing `engine` crate by re-implementing it in engine_traits
//!   and engine_rocks, replacing all the callers with calls into the traits, then delete the
//!   versions in the `engine` crate.
//!
//! - Use the .c() method from engine_rocks::compat::Compat to get a KvEngine reference from Arc<DB>
//!   in the fewest characters. It also works on Snapshot, and can be adapted to other types.
//!
//! - Use `IntoOther` to adapt between error types of dependencies that are not themselves
//!   interdependent. E.g. raft::Error can be created from engine_traits::Error even though neither
//!   `raft` tor `engine_traits` know about each other.
//!
//! - "Plain old data" types in `engine` can be moved directly into `engine_traits` and reexported
//!   from `engine` to ease the transition. Likewise `engine_rocks` can temporarily call code from
//!   inside `engine`.
