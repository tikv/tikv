...
use tidb_qe_datatype::{self, FieldTypeTp};
...
pub fn truncate_binary(s: &mut Vec<u8>, flen: isize) {
    if flen != tidb_qe_datatype::UNSPECIFIED_LENGTH as isize && s.len() > flen as usize {
...
mod tests {
    ...
    fn test_truncate_binary() {
        ...
        let mut s1 = s.clone();
        truncate_binary(&mut s1, tidb_qe_datatype::UNSPECIFIED_LENGTH);
        assert_eq!(s1, s);
        let mut s2 = s.clone();
