...
impl ScalarFunc {
    ...
    pub fn to_base64<'a, 'b: 'a>(
        ...
        if self.field_type.get_flen() == -1
            || self.field_type.get_flen() > tidb_qe_datatype::MAX_BLOB_WIDTH
        {
    ...
    pub fn space<'a, 'b: 'a>(
        ...
        if len > tidb_qe_datatype::MAX_BLOB_WIDTH as usize {
            return Ok(None);
        }
...
fn validate_target_len_for_pad(
    ...
    if !target_len_positive
        || target_len.saturating_mul(size_of_type) > tidb_qe_datatype::MAX_BLOB_WIDTH as usize
...
mod tests {
    ...
    use std::{f64, i64};
    use tidb_qe_datatype::{Collation, FieldTypeFlag, FieldTypeTp, MAX_BLOB_WIDTH};
