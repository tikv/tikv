...
impl<Owner: Fsm> Clone for BasicMailbox<Owner> {
    ...
    fn clone(&self) -> BasicMailbox<Owner> {
        // info!("cloning mailbox"; "count" => Arc::strong_count(&self.state) + 1, "id"
        // => self.id, "backtrace" => format_args!("{:?}",
...
impl<Owner: Fsm> Drop for BasicMailbox<Owner> {
    #[inline]
    fn drop(&mut self) {
        // info!("drop mailbox"; "count" => Arc::strong_count(&self.state) - 1,
        // "id" => self.id, "backtrace" => format_args!("{:?}",
        // backtrace::Backtrace::new()));
    }
}
