    ...
    pub async fn lock_key(&self, key: &Key) -> KeyHandleGuard {
        loop {
            ...
            if entry.value().ptr_eq(&weak2) {
                ...
                // or the `drop` method. It's impossible to have a concurrent `drop` here and
                // `set_table` is only called here. So there is no concurrent access
                // to the `table` field in `KeyHandle`.
                unsafe {
