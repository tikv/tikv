...
impl LazyEtcdClientInner {
    async fn connect(&self) -> Result<EtcdStore> {
        let store = retry(|| {
            ...
            // how to create channels when connecting, hence we cannot update the tls config at
            // runtime. TODO: maybe add some method like `with_channel` for
            // `etcd_client`, and adapt the `SecurityManager` API,       instead of
            // doing everything by own.
