...
use crate::{*, table::sstable::{self, TableIterator}};
...
impl Engine {
    pub fn get_shard_with_ver<'a>(&self, g: &'a epoch::Guard, shard_id: u64, shard_ver: u64) -> Result<&'a Shard> {
        let shard = self.get_shard(shard_id, g).ok_or(Error::ShardNotFound)?;
        if shard.ver != shard_ver {
            warn!("shard {} version not match, current {}, request {}", shard_id, shard.ver, shard_ver);
    ...
    pub async fn split_shard_files(&self, shard_id: u64, shard_ver: u64) -> Result<pb::ChangeSet> {
        ...
        self.split_shard_l0_files(shard, g, cs.mut_split_files()).await?;

        let dfs_opts = dfs::Options::new(shard_id, shard_ver);
        for cf in 0..NUM_CFS {
            let scf = shard.get_cf(cf, g);
            for lh in &scf.levels {
                self.split_tables(dfs_opts, cf, lh, shard.get_split_keys(g), cs.mut_split_files()).await?;
            }
        }
        Ok(cs)
    }
    ...
    async fn split_shard_l0_files<'a>(&self, shard: &'a Shard, g: &'a epoch::Guard, split_files: &mut pb::SplitFiles) -> Result<()> {
    ...
    async fn split_shard_l0_table(&self, shard: &Shard, l0: &sstable::L0Table, split_keys: &Vec<Bytes>) -> Result<Vec<pb::L0Create>> {
        ...
        let mut ids = self.id_allocator.alloc_id(end_keys.len()).map_err(|err| Error::ErrAllocID(err))?;
        for key in &end_keys {
            if let Some((create, data)) = self.build_shard_l0_before_key(
                &mut iters,
                key,
                ids.pop().unwrap(), 
                l0.commit_ts(),
        ...
        for (idx, l0_create) in l0_creates.iter().enumerate() {
            ...
            let future = async move {
                fs.create(id, data, dfs_opts).await
            };
            futures.push(future);
        }
    ...
    fn build_shard_l0_before_key(&self, 
        iters: &mut Vec<Option<Box<sstable::TableIterator>>>, 
        key: &Bytes, 
        id: u64,
        commit_ts: u64,
    ) -> Result<Option<(pb::L0Create, Bytes)>> {
        let builder = sstable::L0Builder::new(id, self.opts.table_builder_options, commit_ts);
    ...
    async fn split_tables(&self, dfs_opts: dfs::Options, cf: usize, lh: &LevelHandler, keys: &Vec<Bytes>, split_files: &mut pb::SplitFiles) -> Result<()> {
        ...
        for tbl in &lh.tables {
            ...
            let mut ids = self.id_allocator.alloc_id(related_keys.len()).map_err(|e| Error::ErrAllocID(e))?;
    ...
    pub fn finish_split(&self, cs: pb::ChangeSet) -> Result<()> {
        ...
        let split = cs.mut_split();
        let new_shards = split.get_new_shards();
    ...
    fn build_split_shards(&self, old_shard: &Shard, g: &epoch::Guard,  split: &pb::Split, sequence: u64) -> Result<()> {
        ...
        for i in 0..split_ctx.mem_tbls.len() {
            let (start_key, end_key) = get_splitting_start_end(old_shard.start.chunk(), old_shard.end.chunk(), split.get_keys(), i);
            let mut new_shard = Shard::new(&new_shard_props[i], new_ver, start_key, end_key, self.opts.clone());
        ...
        let mut new_shard_l0s = vec![vec![]; new_shards.len()];
        for l0 in &l0s.tbls {
            let idx = get_split_shard_index(split.get_keys(), l0.smallest());
            let new_l0s = &mut new_shard_l0s[idx];
        ...
        for cf in 0..NUM_CFS {
            ...
            new_scfs.resize_with(new_shards.len(), || ShardCF::new(self.opts.cfs[cf].max_levels));
            for lh in &old_scf.levels {
                for tbl in &lh.tables {
                    self.insert_table_to_shard(tbl, cf, lh.level, &mut new_scfs, split.get_keys());
                }
            }
