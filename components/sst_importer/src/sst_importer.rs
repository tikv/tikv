...
mod tests {
    ...
    use tikv_util::{
        box_err, codec::stream_event::EventEncoder, resizable_threadpool::ResizableRuntime,
    ...
    fn test_download_files_ext_write_only_short_value_latest_mvcc() {
        ...
        let mut cnt = 0;
        match name_to_cf("write") {
            Some(CF_WRITE) => cnt += 1,
            Some(CF_DEFAULT) => assert_eq!(cnt, 1),
            _ => assert_eq!(cnt, 1),
        }
        match name_to_cf("default") {
            Some(CF_WRITE) => assert_eq!(cnt, 2),
            Some(CF_DEFAULT) => cnt += 2,
            _ => assert_eq!(cnt, 1),
        }
        assert_eq!(cnt, 3);
    }
