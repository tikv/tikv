...
lazy_static! {
    ...
    pub static ref INPORTER_COMPACT_KEYS_COUNT: IntCounterVec = register_int_counter_vec!(
        "tikv_import_compact_keys_count",
        "Bucketed histogram of importer download keys from compacted SST files",
        &["type"]
    ).unwrap();
