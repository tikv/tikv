...
pub struct GroupQuotaAdjustWorker<R> {
    ...
    // Measures unified-read-pool and grpc-server thread CPU usage, feeding
    // the common resource-pressure score alongside whole-process CPU.
    read_pool_cpu_tracker: ThreadGroupCpuTracker,
...
impl<R: ResourceStatsProvider> GroupQuotaAdjustWorker<R> {
    pub fn with_quota_getter(
        ...
        Self {
            ...
            read_pool_cpu_tracker: ThreadGroupCpuTracker::new(UNIFIED_READ_POOL_THREAD),
    ...
    pub fn adjust_quota(&mut self) {
        ...
        let read_pool_cpu_cores = self.read_pool_cpu_tracker.measure_cpu_cores();
        let grpc_cpu_cores = self.grpc_cpu_tracker.measure_cpu_cores();
        self.resource_ctl
            .set_measured_read_pool_cpu_cores(read_pool_cpu_cores);
