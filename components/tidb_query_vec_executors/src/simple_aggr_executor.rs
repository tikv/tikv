...
mod tests {
    ...
    fn test_it_works_unit() {
        ...
        assert_eq!(r.physical_columns[0].decoded().as_int_vec(), &[Some(15)]);
        // Foo(NULL) for 5 rows, so it is 0.
        assert_eq!(r.physical_columns[1].decoded().as_int_vec(), &[Some(0)]);
        // Bar(42.5) for 5 rows, so it is (5, 5, 42.5*5).
        assert_eq!(r.physical_columns[2].decoded().as_int_vec(), &[Some(5)]);
        assert_eq!(r.physical_columns[3].decoded().as_int_vec(), &[Some(5)]);
        assert_eq!(
            r.physical_columns[4].decoded().as_real_vec(),
            &[Real::new(212.5).ok()]
        );
        // Bar(NULL) for 5 rows, so it is (5, 0, 0).
        assert_eq!(r.physical_columns[5].decoded().as_int_vec(), &[Some(5)]);
        assert_eq!(r.physical_columns[6].decoded().as_int_vec(), &[Some(0)]);
        assert_eq!(
            r.physical_columns[7].decoded().as_real_vec(),
            &[Real::new(0.0).ok()]
        );
        // Foo([abc, NULL, "", HelloWorld, aaaaa]) => 3+0+0+10+5
        assert_eq!(r.physical_columns[8].decoded().as_int_vec(), &[Some(18)]);
        // Bar([1.0, 2.0, NULL, 4.5, 4.5]) => (5, 4, 12.0)
        assert_eq!(r.physical_columns[9].decoded().as_int_vec(), &[Some(5)]);
        assert_eq!(r.physical_columns[10].decoded().as_int_vec(), &[Some(4)]);
        assert_eq!(
            r.physical_columns[11].decoded().as_real_vec(),
            &[Real::new(12.0).ok()]
        );
        assert!(r.is_drained.unwrap());
    }
    ...
    fn test_it_works_integration() {
        ...
        assert_eq!(r.physical_columns[2].decoded().as_int_vec(), &[Some(0)]);
        // COUNT([1.0, 2.0, NULL, 4.5, 4.5]) => 4
        assert_eq!(r.physical_columns[3].decoded().as_int_vec(), &[Some(4)]);
        // AVG(42.5) for 5 rows, so it is (5, 212.5). Notice that AVG returns sum.
        assert_eq!(r.physical_columns[4].decoded().as_int_vec(), &[Some(5)]);
        assert_eq!(
            r.physical_columns[5].decoded().as_real_vec(),
            &[Real::new(212.5).ok()]
        );
        // AVG(NULL) for 5 rows, so it is (0, NULL).
        assert_eq!(r.physical_columns[6].decoded().as_int_vec(), &[Some(0)]);
        assert_eq!(r.physical_columns[7].decoded().as_decimal_vec(), &[None]);
        // Foo([NULL, 7.0, NULL, NULL, 1.5]) => (2, 8.5)
        assert_eq!(r.physical_columns[8].decoded().as_int_vec(), &[Some(2)]);
        assert_eq!(
            r.physical_columns[9].decoded().as_real_vec(),
            &[Real::new(8.5).ok()]
        );
        assert!(r.is_drained.unwrap());
    }
    ...
    fn test_no_row() {
        ...
        assert_eq!(r.physical_columns[0].decoded().as_int_vec(), &[Some(42)]);
        assert!(r.is_drained.unwrap());
    }
}
