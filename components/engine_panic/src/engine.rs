...
use engine_traits::{KvEngine, Result, WriteOptions, Peekable, Mutable, ReadOptions, Iterable, Iterator, SeekKey, IterOptions};
...
impl KvEngine for PanicEngine {
    ...
    fn write_opt(&self, opts: &WriteOptions, wb: &Self::WriteBatch) -> Result<()> { panic!() }
    fn write_batch(&self) -> Self::WriteBatch { panic!() }
    fn write_batch_with_cap(&self, cap: usize) -> Self::WriteBatch { panic!() }
    fn snapshot(&self) -> Self::Snapshot { panic!() }
    fn sync(&self) -> Result<()> { panic!() }
    fn cf_names(&self) -> Vec<&str> { panic!() }
    fn bad_downcast<T: 'static>(&self) -> &T { panic!() }
}
...
impl Mutable for PanicEngine {
    fn put_opt(&self, opts: &WriteOptions, key: &[u8], value: &[u8]) -> Result<()> { panic!() }
    fn put_cf_opt(&self, opts: &WriteOptions, cf: &str, key: &[u8], value: &[u8]) -> Result<()> { panic!() }

    fn delete_opt(&self, opts: &WriteOptions, key: &[u8]) -> Result<()> { panic!() }
    fn delete_cf_opt(&self, opts: &WriteOptions, cf: &str, key: &[u8]) -> Result<()> { panic!() }
}
