    ...
    pub async fn next_opt(
        ...
        loop {
            ...
            let some_row = match range {
                IterStatus::NewRange(Range::Point(ref r)) => {
                    ...
                    self.storage
                        .get(self.is_key_only, r.clone(), self.source_stmt.clone())?
                }
                IterStatus::NewRange(Range::Interval(ref r)) => {
                    if self.is_scanned_range_aware {
                ...
                IterStatus::Drained => {
                    ...
                    corr_debug!("scanner next_opt drained";
                        "start_ts" => self.source_stmt.start_ts,
                        "connection id" => self.source_stmt.connection_id,
                        "range" => ?range,
                    );
